...
const EVENT_TYPES = ['meeting', 'holiday', 'birthday', 'anniversary', 'training', 'event', 'other'];
const VISIBILITIES = ['personal', 'company'];
...
export function createCalendarRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            if (visibility === 'company') {
                ...
                if (!ok)
                    return res.status(403).json({ error: 'Forbidden: company events need hr.calendar.manage' });
            }
    ...
    router.delete('/:id', async (req, res) => {
        try {
            const user = req.user;
            const event = await models.calendarEvents.findById(String(req.params.id));
            if (!event)
