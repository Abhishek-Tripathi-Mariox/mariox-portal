...
import { validateLength, validateOptional, validateRange, respondWithError, } from '../validators';
import { createUserNotification } from './notifications';
function toNumber(value, fallback = 0) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
}
export function createSalarySlipsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const [rows, users] = await Promise.all([
                models.salarySlips.find(filter),
                models.users.find({}),
            ]);
    ...
    router.post('/', requireAnyPermission(models, 'hr.salary_slips.manage'), async (req, res) => {
        try {
            ...
            const existing = await models.salarySlips.findOne({ user_id: targetUserId, month });
            ...
            const issuer = await models.users.findById(user.sub);
        ...
        try {
            ...
            for (const e of entries) {
                try {
                    ...
                    if (!targetUserId) {
                        errors.push({ user_id: null, error: 'Missing user_id' });
                        continue;
                    }
                    const targetUser = await models.users.findById(targetUserId);
                    if (!targetUser) {
                        errors.push({ user_id: targetUserId, error: 'Employee not found' });
                        continue;
                    }
