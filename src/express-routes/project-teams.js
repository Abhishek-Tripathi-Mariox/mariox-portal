...
import { validateLength, validateOptional, validateHexColor, respondWithError, } from '../validators';
async function enrichTeams(models, teams) {
    const [users, members, projects] = await Promise.all([
        models.users.find({}),
        models.projectTeamMembers.find({}),
    ...
    return teams.map((t) => {
        const lead = usersById.get(String(t.team_lead_id));
        const project = projectsById.get(String(t.project_id));
...
export function createProjectTeamsRouter(models, jwtSecret) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    router.get('/', async (_req, res) => {
        try {
            const teams = await models.projectTeams.find({});
    ...
    router.get('/project/:projectId', async (req, res) => {
        try {
            ...
            const teams = await models.projectTeams.find({ project_id: projectId });
    ...
    router.post('/', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            if (body.color)
                validateHexColor(body.color, 'Color');
    ...
    router.get('/:teamId', async (req, res) => {
        try {
            const team = await models.projectTeams.findById(String(req.params.teamId));
            if (!team)
    ...
    router.post('/:teamId/members', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            const existing = await models.projectTeamMembers.findOne({ project_team_id: teamId, user_id });
    ...
    router.delete('/:teamId/members/:userId', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            const team = await models.projectTeams.findById(teamId);
...
async function createTeam(models, user, projectId, body) {
    const existing = await models.projectTeams.find(projectId ? { project_id: projectId } : { project_id: null });
