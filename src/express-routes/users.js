...
import { validateEmail, validateNewPassword, validateName, validateEnum, validateOptional, validatePhone, validateLength, validateRange, validatePositiveNumber, validateISODate, validateHexColor, respondWithError, } from '../validators';
...
function parseShift(body) {
    const hhmm = (v) => {
        ...
        if (!/^\d{1,2}:\d{2}$/.test(s))
            throw new Error('Shift time must be HH:mm');
        const [h, m] = s.split(':').map(Number);
        if (h < 0 || h > 23 || m < 0 || m > 59)
            throw new Error('Shift time out of range');
    ...
    const days = (v) => {
        if (v === undefined)
            return null;
        if (!Array.isArray(v))
            throw new Error('shift_days must be an array');
        return v.map((d) => {
            const n = Number(d);
            if (!Number.isInteger(n) || n < 0 || n > 6)
                throw new Error('shift_days entries must be 0..6');
            return n;
        });
    };
    return {
...
async function resolveSalesHierarchy(models, role, body, existing) {
    const r = normalizeRole(role);
    let manager_id = body?.manager_id ? String(body.manager_id).trim() : null;
    let tl_id = body?.tl_id ? String(body.tl_id).trim() : null;
    ...
    if (existing) {
        ...
        if (!('tl_id' in (body || {})))
            tl_id = existing.tl_id ?? null;
    }
    if (r === ROLES.SALES_TL) {
        if (!manager_id)
            throw new Error('Manager is required for a Sales TL');
        const mgr = await models.users.findById(manager_id);
    ...
    if (r === ROLES.SALES_AGENT) {
        if (!tl_id)
            throw new Error('Team Lead is required for a Sales Agent');
        const tl = await models.users.findById(tl_id);
...
function getRoleFilter(role) {
    ...
    if (normalized === 'client')
        return ['client'];
    if (normalized === 'pc' || normalized === 'team')
        return [normalized];
    return null;
}
...
export function createUsersRouter(models, jwtSecret, runtimeEnv = {}) {
    ...
    router.get('/sales-tls', async (req, res) => {
        try {
            const filter = { role: ROLES.SALES_TL, is_active: 1 };
            if (req.query.manager_id)
                filter.manager_id = String(req.query.manager_id);
            const list = await models.users.find(filter);
            list.sort((a, b) => String(a.full_name || '').localeCompare(String(b.full_name || '')));
            return res.json({ data: list, users: list });
        }
        catch {
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const monthlyConsumedByUser = new Map();
            ...
            for (const entry of currentMonthTimesheets) {
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const assignments = await models.projectAssignments.find({ user_id: id, is_active: 1 });
            const projectIds = [...new Set(assignments.map((assignment) => String(assignment.project_id)))];
            const projects = projectIds.length
                ? await models.projects.find({ id: { $in: projectIds } })
                : [];
            const projectsById = new Map(projects.map((project) => [String(project.id), project]));
            const userTimesheets = await models.timesheets.find({ user_id: id });
            const leaves = await models.leaves.find({ user_id: id }, { sort: { start_date: -1 }, limit: 10 });
    ...
    router.post('/', requireRole('admin'), async (req, res) => {
        try {
            ...
            let mail = { sent: false };
            try {
                const loginUrl = String(runtimeEnv.CLIENT_LOGIN_URL || runtimeEnv.APP_URL || runtimeEnv.PUBLIC_BASE_URL || '').trim() ||
                    'http://localhost:3000/';
    ...
    router.put('/:id', requireRole('admin', 'pm'), async (req, res) => {
        try {
            ...
            const body = (req.body || {});
            const existing = await models.users.findById(id);
            if (!existing)
            ...
            if (nextRole === ROLES.SALES_TL &&
                normalizeRole(existing.role) === ROLES.SALES_TL &&
                hierarchy.manager_id !== (existing.manager_id ?? null)) {
                await models.users.updateMany({ tl_id: id, role: ROLES.SALES_AGENT }, { $set: { manager_id: hierarchy.manager_id, updated_at: new Date().toISOString() } });
            }
            const shift = parseShift(body);
            const setPatch = {
                full_name: fullName,
            ...
            if (shift.shift_start !== undefined)
                setPatch.shift_start = shift.shift_start;
            if (shift.shift_end !== undefined)
                setPatch.shift_end = shift.shift_end;
            if (shift.shift_days !== undefined)
                setPatch.shift_days = shift.shift_days;
    ...
    router.delete('/:id', requireRole('admin'), async (req, res) => {
        try {
            ...
            if (!existing)
                return res.status(404).json({ error: 'User not found' });
            const result = await models.users.deleteById(id);
        ...
        try {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                try {
                    ...
                    const role = validateEnum((record.role || 'developer').toLowerCase(), STAFF_CREATE_ROLES, 'Role');
                    const existing = await models.users.findByEmail(email);
                    if (existing) {
