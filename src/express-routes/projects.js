...
import { validateName, validateLength, validateEnum, validateOptional, validateISODate, validatePositiveNumber, respondWithError, } from '../validators';
const PROJECT_TYPES = ['development', 'maintenance', 'support', 'consulting'];
const PROJECT_STATUSES = ['active', 'on_hold', 'completed', 'archived', 'cancelled'];
const PROJECT_PRIORITIES = ['critical', 'high', 'medium', 'low'];
const ASSIGNMENT_TYPES = ['in_house', 'external'];
const ASSIGNEE_TYPES = ['team', 'user'];
const DELIVERY_KINDS = ['app', 'web', 'both'];
...
function applyCommercialVisibility(project, role) {
    if (role === 'admin')
        return project;
    ...
    if (allowed.map((r) => String(r).toLowerCase()).includes(role))
        return project;
    const { sold_by, project_amount, commercial_visible_to, ...rest } = project;
    return rest;
}
function isProjectLinkedToUser(project, user, assignments) {
...
export function createProjectsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const myAssignmentProjectIds = new Set(assignments.filter((a) => String(a.user_id) === String(user?.sub)).map((a) => String(a.project_id)));
            const visibleProjects = canViewAll ? projects : projects.filter((p) => {
                ...
                if (role === 'developer') {
                    ...
                    if (myAssignmentProjectIds.has(String(p.id)))
                        return true;
                    if (p.pm_id === user?.sub || p.pc_id === user?.sub || p.team_lead_id === user?.sub)
                        return true;
                    return false;
                }
            ...
            const enriched = visibleProjects.map((p) => {
                const tl = usersById.get(String(p.team_lead_id));
                const pm = usersById.get(String(p.pm_id));
                const pc = usersById.get(String(p.pc_id));
    ...
    router.get('/next-code', async (req, res) => {
        try {
            ...
            if (!prefix)
                return res.status(400).json({ error: 'kind must be one of: app, web, both' });
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const [users, assignments, timesheets] = await Promise.all([
                models.users.find({}),
                models.projectAssignments.find({ project_id: id, is_active: 1 }),
                models.timesheets.find({ project_id: id }),
            ]);
    ...
    router.get('/:id/developers', async (req, res) => {
        try {
            ...
            if (!project)
                return res.status(404).json({ error: 'Project not found' });
            const assignmentsForScope = await models.projectAssignments.find({ project_id: id, is_active: 1 });
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const commercialVisibleTo = Array.isArray(body.commercial_visible_to)
                ? body.commercial_visible_to.map((r) => String(r).trim().toLowerCase()).filter(Boolean)
                : [];
    ...
    router.post('/import', async (req, res) => {
        try {
            ...
            if (rows.length < 2)
                return res.status(400).json({ error: 'CSV must contain header + data rows' });
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                try {
                    ...
                    const dup = await models.projects.findOne({ code });
                    if (dup) {
                    ...
                    let externalTeamId = null;
                    let externalAssigneeType = null;
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const commercialVisibleTo = Array.isArray(body.commercial_visible_to)
                ? body.commercial_visible_to.map((r) => String(r).trim().toLowerCase()).filter(Boolean)
                : [];
            const assignments = await models.projectAssignments.find({ project_id: id, is_active: 1 });
    ...
    router.delete('/:id/assign/:userId', async (req, res) => {
        try {
            ...
            await models.projectAssignments.updateMany({ project_id: projectId, user_id: userId }, { $set: { is_active: 0, updated_at: new Date().toISOString() } });
    ...
    router.patch('/:id/assign/:userId', async (req, res) => {
        try {
            ...
            await models.projectAssignments.updateMany({ project_id: projectId, user_id: userId }, {
                $set: {
    ...
    router.post('/:id/assign-bulk', async (req, res) => {
        try {
            ...
            if (!Array.isArray(developers))
                return res.status(400).json({ error: 'developers must be array' });
            await models.projectAssignments.updateMany({ project_id: projectId }, { $set: { is_active: 0, updated_at: new Date().toISOString() } });
    ...
    router.post('/:id/notes', async (req, res) => {
        try {
            ...
            if (!project)
                return res.status(404).json({ error: 'Project not found' });
            const assignments = await models.projectAssignments.find({ project_id: projectId, is_active: 1 });
            if (!(await userHasAnyPermission(models, user, 'projects.view_all')) && !isProjectLinkedToUser(project, user, assignments)) {
                return res.status(403).json({ error: 'You do not have access to this project' });
            }
