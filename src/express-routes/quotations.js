...
async function getQuotationPerms(models, user) {
...
function normalizeQuotationPayload(body) {
    ...
    const subtotal = +line_items.reduce((sum, it) => sum + (it.amount || 0), 0).toFixed(2);
...
function renderQuotationHtml(q, leadName, senderName) {
...
function renderQuotationPlainText(q, leadName, senderName) {
    const lines = [];
    lines.push(q.title);
    if (q.client_name || leadName)
        lines.push(`Prepared for ${q.client_name || leadName}`);
    if (q.quote_number)
        lines.push(`Quote #${q.quote_number}`);
    if (q.validity_date)
        lines.push(`Valid till ${q.validity_date}`);
    lines.push('');
    if (q.intro_text) {
        lines.push(q.intro_text);
        lines.push('');
    }
    if ((q.line_items || []).length) {
        lines.push('Line Items:');
        for (const [i, it] of q.line_items.entries()) {
...
export function createQuotationsRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    // ── LIST ─────────────────────────────────────────────────
    router.get('/', async (req, res) => {
        try {
            const quotations = (await models.quotations.find({}));
            ...
            const allSends = (await models.quotationSends.find({}));
    ...
    router.get('/permissions', requireRole('admin'), async (_req, res) => {
        try {
            const grants = (await models.quotationPermissions.find({}));
    ...
    router.post('/permissions', requireRole('admin'), async (req, res) => {
        try {
            ...
            const existing = (await models.quotationPermissions.findOne({ user_id: userId }));
            if (existing)
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const q = (await models.quotations.findOne({ id }));
            if (!q)
    ...
    router.put('/:id', async (req, res) => {
        try {
            const user = req.user;
            const id = String(req.params.id);
            const existing = (await models.quotations.findOne({ id }));
            if (!existing)
                return res.status(404).json({ error: 'Quotation not found' });
    ...
    router.get('/:id/history', async (req, res) => {
        try {
            const id = String(req.params.id);
            const sends = (await models.quotationSends.find({ quotation_id: id }));
    ...
    router.post('/:id/send/:leadId', async (req, res) => {
        try {
            ...
            const q = (await models.quotations.findOne({ id: quotationId }));
            if (!q)
                return res.status(404).json({ error: 'Quotation not found' });
            ...
            const body = req.body || {};
            const subject = validateLength(String(body.subject || `Quotation — ${q.title}${q.quote_number ? ' (' + q.quote_number + ')' : ''}`).trim(), 1, 300, 'Subject');
            ...
            if (q.file?.url) {
                try {
                    ...
                    if (!fileRes.ok)
                        throw new Error(`Failed to fetch quotation file (HTTP ${fileRes.status})`);
            ...
            if (!sentOk)
                return res.status(502).json({ error: sendError || 'Failed to send quotation' });
