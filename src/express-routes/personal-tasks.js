...
async function buildVisibilityFilter(models, user) {
    const role = String(user?.role || '').toLowerCase();
    const uid = String(user?.sub || '');
    if (!uid)
        return { id: '__none__' };
    if (['admin', 'pm', 'pc', 'hr'].includes(role))
        return {};
    const visibleAssignees = new Set([uid]);
    if (role === 'sales_manager') {
        const tls = await models.users.find({ role: 'sales_tl', manager_id: uid }) || [];
        const tlIds = tls.map((t) => String(t.id));
        tlIds.forEach((id) => visibleAssignees.add(id));
        if (tlIds.length) {
            const agents = await models.users.find({ role: 'sales_agent', tl_id: { $in: tlIds } }) || [];
            agents.forEach((a) => visibleAssignees.add(String(a.id)));
        }
    }
    else if (role === 'sales_tl') {
        const agents = await models.users.find({ role: 'sales_agent', tl_id: uid }) || [];
        agents.forEach((a) => visibleAssignees.add(String(a.id)));
    }
    return {
        $or: [
...
export function createPersonalTasksRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const rows = await models.personalTasks.find(filter) || [];
            const userIds = new Set();
            rows.forEach((t) => { if (t.assigned_to)
                userIds.add(String(t.assigned_to)); if (t.created_by)
                userIds.add(String(t.created_by)); });
            const users = userIds.size
                ? await models.users.find({ id: { $in: Array.from(userIds) } }) || []
                : [];
    ...
    router.patch('/:id', async (req, res) => {
        try {
            ...
            const task = await models.personalTasks.findById(id);
            if (!task)
            ...
            if ('title' in body)
                patch.title = validateLength(String(body.title || '').trim(), 1, 200, 'Title');
            if ('description' in body)
                patch.description = String(body.description || '').trim().slice(0, 4000);
            if ('priority' in body)
                patch.priority = validateEnum(body.priority, PRIORITIES, 'Priority');
            if ('due_date' in body)
            ...
            if ('assigned_to' in body && (isCreator || isElevated)) {
                ...
                const assignee = await models.users.findById(next);
                if (!assignee)
                    return res.status(400).json({ error: 'Assignee not found' });
