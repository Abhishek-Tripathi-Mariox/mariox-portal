...
const SEVERITIES = ['verbal', 'written', 'final'];
export function createWarningsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.patch('/:id/acknowledge', async (req, res) => {
        try {
            const user = req.user;
            const w = await models.warnings.findById(String(req.params.id));
            if (!w)
                return res.status(404).json({ error: 'Warning not found' });
            // Only the recipient can acknowledge
            if (w.user_id !== user.sub)
    ...
    router.patch('/:id/respond', async (req, res) => {
        try {
            ...
            if (!w)
                return res.status(404).json({ error: 'Warning not found' });
            if (w.user_id !== user.sub)
                return res.status(403).json({ error: 'Forbidden: not your warning' });
            ...
            if (w.issued_by) {
                const responder = await models.users.findById(user.sub);
                await createUserNotification(models, {
                    user_id: w.issued_by,
