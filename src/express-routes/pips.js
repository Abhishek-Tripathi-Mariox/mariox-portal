...
const PIP_STATUSES = ['draft', 'active', 'completed', 'extended', 'failed', 'cancelled'];
export function createPipsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.post('/', requireAnyPermission(models, 'hr.pips.manage'), async (req, res) => {
        try {
            ...
            const manager = await models.users.findById(user.sub);
    ...
    router.patch('/:id', requireAnyPermission(models, 'hr.pips.manage'), async (req, res) => {
        try {
            const body = req.body || {};
            const pip = await models.pips.findById(String(req.params.id));
            if (!pip)
                return res.status(404).json({ error: 'PIP not found' });
            const patch = { updated_at: new Date().toISOString() };
            if (body.status)
                patch.status = validateEnum(body.status, PIP_STATUSES, 'Status');
