...
const UPDATE_TYPES = ['general', 'milestone', 'release', 'risk', 'announcement'];
export function createActivityRouter(models, jwtSecret) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    router.get('/', async (req, res) => {
        try {
            const { project_id, entity_type, limit = '30', client_id } = req.query;
            const filter = {};
            if (project_id)
            ...
            if (client_id) {
                const clientProjects = await models.projects.find({ client_id });
                filter.project_id = { $in: clientProjects.map((p) => p.id) };
            }
            const [logs, projects] = await Promise.all([
                models.activityLogs.find(filter),
    ...
    router.get('/project/:project_id/feed', async (req, res) => {
        try {
            ...
            const [logs, comments, updates, users] = await Promise.all([
                models.activityLogs.find({ project_id: projectId }),
                models.comments.find({ entity_type: 'project', entity_id: projectId }),
                models.projectUpdates.find({ project_id: projectId }),
                models.users.find({}),
            ]);
            ...
            const enrichedUpdates = updates
                ...
                .slice(0, 10)
                .map((u) => {
                const author = usersById.get(String(u.posted_by));
                return {
                    ...u,
