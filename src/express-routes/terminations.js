...
const TERMINATION_TYPES = ['resignation', 'dismissal', 'layoff', 'retirement', 'contract_end', 'other'];
const TERMINATION_STATUSES = ['initiated', 'notice_period', 'completed', 'cancelled'];
export function createTerminationsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            if (!isManager) {
                ...
                const rows = await models.terminations.find({ user_id: user.sub });
            ...
            const [rows, users] = await Promise.all([
                models.terminations.find(filter),
                models.users.find({}),
            ]);
    ...
    router.patch('/:id/checklist', requireAnyPermission(models, 'hr.terminations.manage'), async (req, res) => {
        try {
            const term = await models.terminations.findById(String(req.params.id));
            if (!term)
            ...
            for (const key of allowed) {
                if (body.checklist[key] !== undefined)
                    sanitized[key] = Boolean(body.checklist[key]);
            }
    ...
    router.patch('/:id', requireAnyPermission(models, 'hr.terminations.manage'), async (req, res) => {
        try {
            ...
            if (body.status)
                patch.status = validateEnum(body.status, TERMINATION_STATUSES, 'Status');
