...
// installs.
async function getScopePerms(models, user) {
    ...
    let perms = [];
    if (role) {
        const roleDoc = (await models.roles.findOne({ key: role }));
...
function normalizeBlocks(raw) {
    ...
    for (const b of raw) {
        ...
        if (type === 'paragraph') {
            ...
            if (text)
                out.push({ type: 'subheading', text });
        }
        else if (type === 'bullets' || type === 'numbered') {
            const items = (Array.isArray(b.items) ? b.items : [])
                .map((x) => String(x || '').trim().slice(0, 800))
                .filter(Boolean);
            if (items.length)
                out.push({ type: type, items });
        }
        else if (type === 'table') {
            ...
            const rows = rowsRaw
                .map((r) => (Array.isArray(r) ? r : []).map((c) => String(c || '').slice(0, 2000)))
...
function renderScopeBlockHtml(b, esc, paragraph) {
    if (b.type === 'paragraph') {
...
function renderScopeHtml(scope, leadName, senderName) {
    ...
    const sectionsHtml = (scope.sections || []).map((s, idx) => {
        ...
        const blocksHtml = (s.blocks || []).map((b) => renderScopeBlockHtml(b, esc, paragraph)).join('');
    ...
    <div style="margin-top:18px">
      ...
      <ul style="margin:8px 0;padding-left:22px;color:#374151;font:14px/1.6 Arial,Helvetica,sans-serif">
        ${(scope.deliverables || []).map((d) => `<li style="margin-bottom:4px">${esc(d)}</li>`).join('')}
      </ul>
    </div>` : '';
    ...
    const headerLines = [];
    if (scope.client_name || leadName)
        headerLines.push(`<div><strong style="color:#1f2937">Client:</strong> <span style="color:#374151">${esc(scope.client_name || leadName)}</span></div>`);
    if (scope.spoc_name)
        headerLines.push(`<div><strong style="color:#1f2937">SPOC:</strong> <span style="color:#374151">${esc(scope.spoc_name)}</span></div>`);
...
function renderScopeBlockText(b) {
    if (b.type === 'paragraph')
        return b.text + '\n';
    if (b.type === 'subheading')
        return `\n— ${b.text} —\n`;
    ...
    if (b.type === 'table') {
        const widths = (b.columns.length ? [b.columns, ...b.rows] : b.rows).reduce((acc, row) => {
            row.forEach((cell, i) => { acc[i] = Math.max(acc[i] || 0, Math.min(36, String(cell || '').length)); });
            return acc;
        }, []);
        const fmt = (row) => row.map((c, i) => String(c || '').slice(0, 36).padEnd(widths[i] || 0)).join(' | ');
        const lines = [];
        if (b.columns.length) {
...
function renderScopePlainText(scope, leadName, senderName) {
    const lines = [];
    lines.push('STATEMENT OF WORK (SOW)');
    if (scope.project_name || scope.title)
        lines.push(scope.project_name || scope.title);
    lines.push('');
    if (scope.client_name || leadName)
        lines.push(`Client: ${scope.client_name || leadName}`);
    if (scope.spoc_name)
        lines.push(`SPOC: ${scope.spoc_name}`);
    ...
    for (const [idx, s] of (scope.sections || []).entries()) {
        if (s.heading)
            lines.push(`${idx + 1}. ${String(s.heading).toUpperCase()}`);
        if (s.body)
            lines.push(s.body);
        for (const b of (s.blocks || []))
            lines.push(renderScopeBlockText(b));
        lines.push('');
    }
    ...
    if (scope.timeline_text) {
        lines.push('TIMELINE');
        lines.push(scope.timeline_text);
        lines.push('');
    }
    ...
    if (scope.footer_text) {
        lines.push(scope.footer_text);
        lines.push('');
    }
    lines.push('Regards,');
...
export function createScopesRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    // ── LIST ─────────────────────────────────────────────────
    router.get('/', async (req, res) => {
        try {
            const scopes = (await models.scopes.find({}));
            ...
            const creators = creatorIds.length
                ? (await models.users.find({ id: { $in: creatorIds } }))
                : [];
            const creatorsById = new Map(creators.map((u) => [String(u.id), u]));
            const allSends = (await models.scopeSends.find({}));
    ...
    router.get('/permissions', requireRole('admin'), async (_req, res) => {
        try {
            const grants = (await models.scopePermissions.find({}));
            const userIds = [...new Set(grants.map((g) => String(g.user_id || '')).filter(Boolean))];
            const users = userIds.length
                ? (await models.users.find({ id: { $in: userIds } }))
                : [];
    ...
    router.post('/permissions', requireRole('admin'), async (req, res) => {
        try {
            ...
            const target = (await models.users.findById(userId));
            if (!target)
                return res.status(404).json({ error: 'User not found' });
            const existing = (await models.scopePermissions.findOne({ user_id: userId }));
            if (existing)
                return res.json({ message: 'Already granted' });
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const scope = (await models.scopes.findOne({ id }));
            if (!scope)
    ...
    router.put('/:id', async (req, res) => {
        try {
            const user = req.user;
            const id = String(req.params.id);
            const existing = (await models.scopes.findOne({ id }));
            if (!existing)
                return res.status(404).json({ error: 'Scope not found' });
    ...
    router.get('/:id/history', async (req, res) => {
        try {
            const id = String(req.params.id);
            const sends = (await models.scopeSends.find({ scope_id: id }));
            ...
            const [leads, senders] = await Promise.all([
                leadIds.length ? models.leads.find({ id: { $in: leadIds } }) : Promise.resolve([]),
                senderIds.length ? models.users.find({ id: { $in: senderIds } }) : Promise.resolve([]),
            ]);
    ...
    router.post('/:id/send/:leadId', async (req, res) => {
        try {
            ...
            const scope = (await models.scopes.findOne({ id: scopeId }));
            if (!scope)
                return res.status(404).json({ error: 'Scope not found' });
            const lead = (await models.leads.findOne({ id: leadId }));
            if (!lead)
                return res.status(404).json({ error: 'Lead not found' });
            const body = req.body || {};
            const subject = validateLength(String(body.subject || `Scope of Work — ${scope.title}`).trim(), 1, 300, 'Subject');
            ...
            if (scope.file?.url) {
                try {
                    ...
                    if (!fileRes.ok)
                        throw new Error(`Failed to fetch SOW file (HTTP ${fileRes.status})`);
            ...
            if (!sentOk)
                return res.status(502).json({ error: sendError || 'Failed to send scope' });
