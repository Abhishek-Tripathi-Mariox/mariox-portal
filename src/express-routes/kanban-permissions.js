...
export function createKanbanPermissionsRouter(models, jwtSecret) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    router.get('/project/:projectId', async (req, res) => {
        try {
            const rows = await models.kanbanPermissions.find({ project_id: req.params.projectId });
    ...
    router.get('/project/:projectId/mine', async (req, res) => {
        try {
            ...
            const perms = await models.kanbanPermissions.findOne({ project_id: projectId, role });
    ...
    router.put('/project/:projectId', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            for (const p of permissions) {
                const existing = await models.kanbanPermissions.findOne({ project_id: projectId, role: p.role });
                const patch = {
