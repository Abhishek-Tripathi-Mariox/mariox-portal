...
import { validateName, validateLength, validateOptional, validateISODate, validatePositiveNumber, respondWithError, } from '../validators';
import { createUserNotification, createUserNotifications } from './notifications';
const AUCTION_STATUSES = ['open', 'awarded', 'cancelled', 'closed'];
const SUBMISSION_STATUSES = ['submitted', 'awarded', 'lost', 'withdrawn'];
...
function canSeeAllSubmissions(viewerRole, auction) {
...
function enrichSubmission(sub, usersById) {
...
export function createBidsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const [auctions, submissions, users] = await Promise.all([
                models.bidAuctions.find({}),
                models.bidSubmissions.find({}),
                models.users.find({}),
            ]);
            const usersById = new Map(users.map((u) => [String(u.id), u]));
            const subsByAuction = new Map();
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const role = String(user?.role || '').toLowerCase();
            const isAdminLike = ['admin', 'pm', 'pc'].includes(role);
            const auction = await models.bidAuctions.findById(String(req.params.id));
            if (!auction)
            ...
            const [subs, users] = await Promise.all([
                models.bidSubmissions.find({ auction_id: auction.id }),
                models.users.find({}),
            ]);
            const usersById = new Map(users.map((u) => [String(u.id), u]));
    ...
    router.post('/', requireRole('admin', 'pm'), async (req, res) => {
        try {
            ...
            const attachments = Array.isArray(body.attachments)
                ? body.attachments
                    .filter((a) => a && typeof a === 'object' && a.file_url)
                    .map((a) => ({
            ...
            const invitedTrimmed = Array.from(new Set(invitedRaw.map((v) => String(v || '').trim()).filter((s) => s.length > 0)));
            ...
            try {
                const admins = await models.users.find({ role: 'admin', is_active: 1 });
                const recipientIds = Array.from(new Set([
                    ...invitedTrimmed,
                    ...admins.map((a) => String(a.id)),
                ]));
                const creator = await models.users.findById(user?.sub);
    ...
    router.put('/:id', requireRole('admin', 'pm'), async (req, res) => {
        try {
            ...
            const auction = await models.bidAuctions.findById(id);
            if (!auction)
                return res.status(404).json({ error: 'Auction not found' });
            if (auction.status !== 'open') {
            ...
            if (body.name !== undefined)
                $set.name = validateName(body.name, 'Auction name', 2, 120);
            if (body.scope !== undefined)
                $set.scope = validateLength(String(body.scope || '').trim(), 1, 20000, 'Project scope');
            ...
            if (body.client_id !== undefined)
                $set.client_id = body.client_id || null;
            if (body.client_name !== undefined)
                $set.client_name = body.client_name || null;
            if (Array.isArray(body.invited_user_ids)) {
                const cleaned = Array.from(new Set(body.invited_user_ids.map((v) => String(v || '').trim()).filter((s) => s.length > 0)));
                if (cleaned.length === 0)
                    return res.status(400).json({ error: 'At least one invitee required' });
    ...
    router.post('/:id/submissions', async (req, res) => {
        try {
            ...
            if (!auction)
                return res.status(404).json({ error: 'Auction not found' });
            if (auction.status !== 'open')
                return res.status(400).json({ error: 'Auction is no longer open' });
            ...
            const existing = await models.bidSubmissions.findOne({ auction_id: id, user_id: user.sub });
            const nowIso = new Date().toISOString();
            const submissionId = existing?.id || generateId('sub');
            const bidder = await models.users.findById(user.sub);
    ...
    router.delete('/:id/submissions/:subId', async (req, res) => {
        try {
            const user = req.user;
            const sub = await models.bidSubmissions.findById(String(req.params.subId));
            if (!sub)
                return res.status(404).json({ error: 'Bid not found' });
            if (sub.user_id !== user.sub)
                return res.status(403).json({ error: 'Not your bid' });
            if (sub.status !== 'submitted')
                return res.status(400).json({ error: 'Awarded bids cannot be withdrawn' });
    ...
    router.post('/:id/submissions/:subId/award', requireRole('admin', 'pm'), async (req, res) => {
        try {
            ...
            const auction = await models.bidAuctions.findById(auctionId);
            if (!auction)
                return res.status(404).json({ error: 'Auction not found' });
            if (auction.status === 'awarded') {
                return res.status(400).json({ error: 'Auction has already been awarded' });
            }
            const sub = await models.bidSubmissions.findById(subId);
            if (!sub || sub.auction_id !== auctionId)
                return res.status(404).json({ error: 'Bid not found' });
            if (sub.status !== 'submitted')
                return res.status(400).json({ error: 'Bid is not in a submitted state' });
            const winner = await models.users.findById(sub.user_id);
            ...
            let endDate = auction.planned_end_date || null;
            ...
            if (bidAttachments.length) {
                await models.documents.insertMany(bidAttachments.map((f) => ({
                    id: generateId('doc'),
                    project_id: projectId,
                    client_id: auction.client_id || null,
            ...
            await models.bidSubmissions.updateMany({ auction_id: auctionId, id: { $ne: subId }, status: 'submitted' }, { $set: { status: 'lost', updated_at: nowIso } });
            // 5) Notifications — only related people. Losing bidders + winner.
            const otherSubs = await models.bidSubmissions.find({ auction_id: auctionId });
            const losingIds = Array.from(new Set(otherSubs.map((s) => s.user_id).filter((uid) => uid && uid !== sub.user_id)));
            if (losingIds.length) {
