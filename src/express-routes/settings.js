...
export function createSettingsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/tech-stacks', async (_req, res) => {
        try {
            const items = await models.techStacks.find({});
            items.sort((a, b) => {
                const cat = String(a.category || '').localeCompare(String(b.category || ''));
                return cat !== 0 ? cat : String(a.name || '').localeCompare(String(b.name || ''));
            });
    ...
    router.get('/holidays', async (_req, res) => {
        try {
            const items = await models.holidays.find({});
    ...
    async function upsertSettings(body) {
        ...
        const existing = await models.settings.findOne({});
        const payload = {
    ...
    router.post('/roles', requireRole('admin'), async (req, res) => {
        try {
            ...
            const existing = await models.roles.findOne({ key });
            if (existing)
    ...
    router.put('/roles/:id', requireRole('admin'), async (req, res) => {
        try {
            const role = await models.roles.findById(String(req.params.id));
            if (!role)
    ...
    router.delete('/roles/:id', requireRole('admin'), async (req, res) => {
        try {
            ...
            const usersWithRole = await models.users.find({ role: role.key });
