...
import { validateLoginInput, validateNewPassword, validateRequired, respondWithError, } from '../validators';
...
async function loadRolePermissions(models, role) {
    const key = String(role || '').toLowerCase().trim();
    if (!key || key === 'admin')
        return [];
    try {
        const doc = (await models.roles.findOne({ key }));
...
export function createAuthRouter(models, jwtSecret, passwordSalt) {
    const router = createRouter();
    const authMiddleware = createAuthMiddleware(jwtSecret);
    router.post('/login', async (req, res) => {
        try {
            const { email, password } = validateLoginInput(req.body?.email, req.body?.password);
            const user = await models.users.findActiveByEmail(email);
            if (!user) {
    ...
    router.post('/verify', async (req, res) => {
        try {
            ...
            const payload = (await jwtVerify(token, encoder.encode(jwtSecret))).payload;
    ...
    router.post('/impersonate/:userId', authMiddleware, async (req, res) => {
        try {
            ...
            const target = await models.users.findActiveById(targetId);
            if (!target)
            ...
            const requester = await models.users.findById(String(ctx?.sub || ''));
            const payload = {
                sub: target.id,
                email: target.email,
    ...
    router.post('/end-impersonation', authMiddleware, async (req, res) => {
        try {
            ...
            const original = await models.users.findActiveById(String(ctx.imp_sub));
            if (!original) {
    ...
    router.post('/change-password', authMiddleware, async (req, res) => {
        try {
            ...
            const user = await models.users.findById(userCtx.sub);
            if (!user)
    ...
    router.post('/admin-reset-password', authMiddleware, requireRole('admin'), async (req, res) => {
        try {
            ...
            try {
                const admin = await models.users.findById(adminCtx.sub);
