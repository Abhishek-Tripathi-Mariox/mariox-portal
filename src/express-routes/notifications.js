 ...
 */
export async function createUserNotification(models, payload) {
    if (!payload.user_id)
        return null;
    try {
        const doc = {
 ...
 */
export async function createUserNotifications(models, userIds, payload) {
    const dedup = new Set();
    for (const id of userIds) {
        if (!id)
            continue;
        if (payload.actor_id && String(id) === String(payload.actor_id))
            continue;
        dedup.add(String(id));
    }
    await Promise.all(Array.from(dedup).map((uid) => createUserNotification(models, { ...payload, user_id: uid })));
}
export function createNotificationsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/stream', async (req, res) => {
        ...
        const onNotif = (doc) => {
            if (!doc || String(doc.user_id) !== userId)
                return;
            try {
    ...
    router.get('/me', async (req, res) => {
        try {
            ...
            const items = await models.userNotifications.find({ user_id: user.sub }, { sort: { created_at: -1 }, limit });
    ...
    router.get('/unread-count', async (req, res) => {
        try {
            ...
            if (!user?.sub)
                return res.status(401).json({ error: 'Unauthenticated' });
            const items = await models.userNotifications.find({ user_id: user.sub, is_read: { $ne: 1 } }, { sort: { created_at: -1 }, limit: 100 });
    ...
    router.post('/:id/read', async (req, res) => {
        try {
            const user = req.user;
            const item = await models.userNotifications.findById(String(req.params.id));
            if (!item)
    ...
    router.post('/read-all', async (req, res) => {
        try {
            ...
            if (!user?.sub)
                return res.status(401).json({ error: 'Unauthenticated' });
            await models.userNotifications.updateMany({ user_id: user.sub, is_read: { $ne: 1 } }, { $set: { is_read: 1, read_at: new Date().toISOString() } });
            return res.json({ success: true });
        }
        catch (error) {
            return res.status(500).json({ error: error?.message || 'Failed' });
