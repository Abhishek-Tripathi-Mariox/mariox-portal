...
const ASSET_TYPES = ['laptop', 'desktop', 'monitor', 'phone', 'sim', 'id_card', 'access_card', 'headset', 'keyboard', 'mouse', 'other'];
const ASSET_STATUSES = ['available', 'assigned', 'returned', 'retired', 'lost'];
export function createHrAssetsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            else if (typeof req.query.assigned_to === 'string')
                filter.assigned_to = req.query.assigned_to;
            const [rows, users] = await Promise.all([
                models.hrAssets.find(filter),
                models.users.find({}),
            ]);
    ...
    router.post('/:id/assign', requireAnyPermission(models, 'hr.assets.manage'), async (req, res) => {
        try {
            ...
            const asset = await models.hrAssets.findById(String(req.params.id));
            if (!asset)
                return res.status(404).json({ error: 'Asset not found' });
            if (asset.status === 'assigned')
                return res.status(400).json({ error: 'Asset is already assigned — return it first' });
    ...
    router.patch('/:id', requireAnyPermission(models, 'hr.assets.manage'), async (req, res) => {
        try {
            ...
            if (body.status)
                patch.status = validateEnum(body.status, ASSET_STATUSES, 'Status');
            if (body.name !== undefined)
                patch.name = validateLength(String(body.name).trim(), 1, 200, 'Name');
