...
import { validateLength, validateEnum, validateOptional, validateISODate, validatePositiveNumber, validateRequired, respondWithError, } from '../validators';
import { createUserNotifications } from './notifications';
const TASK_TYPES = ['task', 'bug', 'feature', 'epic', 'story', 'subtask'];
const TASK_PRIORITIES = ['critical', 'high', 'medium', 'low'];
async function ensureColumns(models, projectId) {
...
async function checkKanbanPerm(models, projectId, userRole, userId, requiredPerm, taskAssigneeId) {
    if (userRole === 'admin' || userRole === 'pm' || userRole === 'pc')
        return { allowed: true };
    if ((userRole === 'developer' || userRole === 'team') && requiredPerm !== 'can_comment') {
        ...
        if (!a) {
            ...
            const ownsExternally = project && (String(project.external_team_id || '') === String(userId) ||
                String(project.awarded_to_user_id || '') === String(userId) ||
                String(project.pm_id || '') === String(userId) ||
                String(project.pc_id || '') === String(userId));
            if (!ownsExternally)
                return { allowed: false, reason: 'not_assigned_to_project' };
        }
    }
...
export function createTasksRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/columns/:project_id', async (req, res) => {
        try {
            ...
            const cols = await models.kanbanColumns.find({ project_id: projectId });
    ...
    router.post('/columns/:project_id', async (req, res) => {
        try {
            const user = req.user;
            if (!(await userHasAnyPermission(models, user, 'projects.manage_kanban_perms')))
            ...
            const existing = await models.kanbanColumns.find({ project_id: projectId });
    ...
    router.delete('/columns/:id', async (req, res) => {
        try {
            ...
            await models.tasks.updateMany({ project_id: col.project_id, status: col.status_key }, { $set: { status: 'backlog', updated_at: new Date().toISOString() } });
    ...
    router.get('/board/:project_id', async (req, res) => {
        try {
            ...
            const [columnDefs, users, sprints, tasksList, subtasksAll, comments] = await Promise.all([
                models.kanbanColumns.find({ project_id: projectId }),
                models.users.find({}),
                models.sprints.find({ project_id: projectId }),
                models.tasks.find(sprintId
                    ? { project_id: projectId, sprint_id: sprintId }
                    : { project_id: projectId }),
                models.tasks.find({ project_id: projectId }),
                models.comments.find({ entity_type: 'task' }),
            ]);
            ...
            for (const task of topLevel) {
                const assignee = usersById.get(String(task.assignee_id));
                const reporter = usersById.get(String(task.reporter_id));
                const sprint = sprintsById.get(String(task.sprint_id));
                const enriched = {
                    ...task,
                ...
                if (task.status in columns)
                    columns[task.status].push(enriched);
                else if ('backlog' in columns)
                    columns.backlog.push(enriched);
            }
    ...
    router.get('/my', async (req, res) => {
        try {
            ...
            const [tasksList, projects, sprints] = await Promise.all([
                models.tasks.find({ assignee_id: user.sub, status: { $ne: 'done' } }),
            ...
            const enriched = tasksList.map((t) => {
                const p = projectsById.get(String(t.project_id));
                const s = sprintsById.get(String(t.sprint_id));
                return {
                    ...t,
    ...
    router.get('/', async (req, res) => {
        try {
            const { project_id, sprint_id, milestone_id, assignee_id, status, priority, type } = req.query;
            const filter = {};
            if (project_id)
            ...
            const enriched = tasksList
                .filter((t) => !t.parent_task_id)
                .map((t) => {
                const a = usersById.get(String(t.assignee_id));
                const r = usersById.get(String(t.reporter_id));
                const p = projectsById.get(String(t.project_id));
                const s = sprintsById.get(String(t.sprint_id));
                return {
                    ...t,
    ...
    router.get('/:id', async (req, res) => {
        try {
            const id = req.params.id;
            const task = await models.tasks.findById(id);
            if (!task)
                return res.status(404).json({ error: 'Task not found' });
            const [subtasks, comments, users, projects, sprints] = await Promise.all([
                models.tasks.find({ parent_task_id: id }),
                models.comments.find({ entity_type: 'task', entity_id: id }),
                models.users.find({}),
                models.projects.find({}),
                models.sprints.find({}),
            ]);
            ...
            const activity = await models.activityLogs.find({ entity_type: 'task', entity_id: id });
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            if (!perm.allowed)
                return res.status(403).json({ error: 'You do not have permission to create tasks on this board', reason: perm.reason });
    ...
    router.put('/:id', async (req, res) => {
        try {
            const user = req.user;
            const id = req.params.id;
            const oldTask = await models.tasks.findById(id);
            if (!oldTask)
                return res.status(404).json({ error: 'Task not found' });
            ...
            if (!perm.allowed)
                return res.status(403).json({ error: 'You do not have permission to edit this task', reason: perm.reason });
            const body = req.body || {};
            ...
            for (const k of allowed) {
                if (k in body)
                    patch[k] = k === 'labels' && Array.isArray(body[k]) ? JSON.stringify(body[k]) : body[k];
            }
            ...
            if (milestoneId && (statusChanged || pctChanged)) {
                try {
                    const allMilestoneTasks = await models.tasks.find({ milestone_id: milestoneId });
                    let completionPct = 0;
                    for (const t of allMilestoneTasks) {
                        ...
                        if (tStatus === 'done' && Number.isFinite(tPct))
                            completionPct += tPct;
                    }
    ...
    router.delete('/:id', async (req, res) => {
        try {
            ...
            if (!perm.allowed)
                return res.status(403).json({ error: 'You do not have permission to delete tasks', reason: perm.reason });
    ...
    router.patch('/:id/move', async (req, res) => {
        try {
            ...
            if (!perm.allowed)
                return res.status(403).json({ error: 'You do not have permission to move tasks', reason: perm.reason });
    ...
    router.post('/:id/comment', async (req, res) => {
        try {
            ...
            const mentionIds = Array.isArray(mention_user_ids)
                ? Array.from(new Set(mention_user_ids.map((v) => String(v || '').trim()).filter(Boolean)))
                : [];
            const comment = {
                id,
                entity_type: 'task',
            ...
            if (mentionIds.length) {
                try {
                    ...
                    if (recipients.length) {
                        const author = user?.sub ? await models.users.findById(user.sub) : null;
