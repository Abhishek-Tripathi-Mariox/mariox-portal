...
export function createAlertsRouter(models, jwtSecret) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    router.get('/', async (req, res) => {
        try {
            const user = req.user;
            const query = { is_dismissed: { $ne: 1 } };
            if (user.role === 'developer') {
                query.$or = [{ user_id: user.sub }, { user_id: null }, { user_id: { $exists: false } }];
            }
            const alerts = await models.alerts.find(query, { sort: { created_at: -1 }, limit: 50 });
    ...
    router.post('/generate', requireRole('admin', 'pm'), async (req, res) => {
        try {
            ...
            const projects = await models.projects.find({ status: 'active' });
            const timesheets = await models.timesheets.find({});
            const users = await models.users.find({ role: 'developer', is_active: 1 });
            const newAlerts = [];
            ...
            if (newAlerts.length)
                await models.alerts.insertMany(newAlerts);
