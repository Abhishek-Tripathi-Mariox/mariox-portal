...
async function buildRevenueByAgentPeriod(models) {
    const [leads, projects] = await Promise.all([
        models.leads.find({}),
        models.projects.find({}),
    ]);
    const leadAssignee = new Map();
    for (const l of leads)
        leadAssignee.set(String(l.id), String(l.assigned_to || ''));
    const out = new Map();
...
async function hasPermission(models, user, key) {
...
export function createSalesIncentivesRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/summary', async (req, res) => {
        try {
            ...
            let agents = allUsers.filter((u) => SALES_ROLES.includes(lower(u.role)) && Number(u.is_active || 0) === 1);
            if (!canViewAll)
                agents = agents.filter((u) => String(u.id) === myId);
            ...
            const records = (await models.salesIncentives.find({ period }));
            ...
            const payments = (await models.salesIncentivePayments.find({ period }));
            const paymentsByUser = new Map();
            for (const p of payments) {
                const uid = String(p.user_id);
                if (!paymentsByUser.has(uid))
                    paymentsByUser.set(uid, []);
            ...
            const rows = agents.map((u) => {
                ...
                const userPayments = (paymentsByUser.get(String(u.id)) || []).sort((a, b) => String(b.paid_at || '').localeCompare(String(a.paid_at || '')));
    ...
    router.post('/:userId/:period/override', async (req, res) => {
        try {
            ...
            let achievedOverride = null;
            if (wantsOverride) {
                if (!canOverride)
                    return res.status(403).json({ error: 'Not allowed to override achieved value' });
                if (body.achieved_override === null || body.achieved_override === '') {
                    ...
                    if (!Number.isFinite(n) || n < 0)
                        return res.status(400).json({ error: 'achieved_override must be a non-negative number' });
                    achievedOverride = n;
                }
            }
            ...
            if (wantsTarget) {
                if (!canSetTarget)
                    return res.status(403).json({ error: 'Not allowed to change the period target' });
                const n = Number(body.target_snapshot);
                if (!Number.isFinite(n) || n < 0)
                    return res.status(400).json({ error: 'target_snapshot must be a non-negative number' });
                targetSnapshot = n;
            }
            if (wantsRate) {
                if (!canSetTarget)
                    return res.status(403).json({ error: 'Not allowed to change the period rate' });
                const n = Number(body.rate_snapshot);
                if (!Number.isFinite(n) || n < 0)
                    return res.status(400).json({ error: 'rate_snapshot must be a non-negative number' });
                rateSnapshot = n;
            }
            const notes = body.notes !== undefined ? String(body.notes).slice(0, 1000) : undefined;
            const nowIso = new Date().toISOString();
            const existing = (await models.salesIncentives.findOne({ user_id: userId, period }));
            if (existing) {
                const patch = { updated_at: nowIso };
                if (wantsOverride)
                    patch.achieved_override = achievedOverride;
                if (wantsTarget)
                    patch.target_snapshot = targetSnapshot;
                if (wantsRate)
                    patch.rate_snapshot = rateSnapshot;
                if (notes !== undefined)
                    patch.notes = notes;
    ...
    router.post('/:userId/:period/payments', async (req, res) => {
        try {
            ...
            if (!PERIOD_RE.test(period))
                return res.status(400).json({ error: 'period must be YYYY-MM' });
            const targetUser = (await models.users.findById(userId));
            if (!targetUser)
                return res.status(404).json({ error: 'User not found' });
            const body = req.body || {};
    ...
    router.get('/history/:userId', async (req, res) => {
        try {
            ...
            const records = (await models.salesIncentives.find({ user_id: userId }));
            ...
            const revenueByAgentPeriod = await buildRevenueByAgentPeriod(models);
            const myRevenueByPeriod = revenueByAgentPeriod.get(userId) || new Map();
            for (const p of myRevenueByPeriod.keys())
                periodsSet.add(p);
            ...
            const allPayments = (await models.salesIncentivePayments.find({ user_id: userId }));
            const paymentsByPeriod = new Map();
            for (const p of allPayments) {
                const k = String(p.period);
                if (!paymentsByPeriod.has(k))
                    paymentsByPeriod.set(k, []);
            ...
            const rows = Array.from(periodsSet)
                ...
                const periodPayments = (paymentsByPeriod.get(period) || []).sort((a, b) => String(b.paid_at || '').localeCompare(String(a.paid_at || '')));
