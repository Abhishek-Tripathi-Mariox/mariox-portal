...
import { validateLength, validateEmail, validateNewPassword, respondWithError, } from '../validators';
...
async function buildLeadVisibilityFilter(models, user) {
    const role = lower(user?.role);
    const userId = String(user?.sub || user?.id || '');
    if (LEADS_GLOBAL_ROLES.includes(role))
        return null;
    ...
    if (role === ROLES.SALES_TL) {
        ...
        const agents = await models.users.find({ tl_id: userId });
...
export async function canUserAccessLead(models, user, lead) {
    if (!lead)
        return false;
    ...
    const set = filter.assigned_to?.$in;
    if (Array.isArray(set))
        return set.includes(String(lead.assigned_to));
    if (filter.assigned_to)
        return String(lead.assigned_to) === String(filter.assigned_to);
    return false;
}
async function logLeadActivity(models, leadId, user, kind, summary, meta = {}) {
...
function sanitizeStatusKey(input) {
    return String(input || '')
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9_]+/g, '_')
...
async function enrichLeads(models, leads) {
    if (!leads.length)
        return [];
    const users = await models.users.find({});
    const usersById = new Map(users.map((u) => [String(u.id), u]));
    const leadIds = leads.map((l) => String(l.id));
    const tasks = await models.leadTasks.find({ lead_id: { $in: leadIds } });
    ...
    return leads.map((l) => {
        const assignee = usersById.get(String(l.assigned_to));
        const creator = usersById.get(String(l.created_by));
        return {
            ...l,
            ...
            tasks: (tasksByLead.get(String(l.id)) || []).sort((a, b) => String(a.due_date || '').localeCompare(String(b.due_date || ''))),
        };
    });
}
export function createLeadsRouter(models, jwtSecret, runtimeEnv = {}, passwordSalt = '') {
    const router = Router();
    ...
    function statusRepo(kind) {
        if (kind === 'lead')
            return models.leadStatuses;
        if (kind === 'task')
            return models.leadTaskStatuses;
        return null;
    }
    router.get('/statuses', async (_req, res) => {
        try {
            const [leadStatuses, taskStatuses] = await Promise.all([
                models.leadStatuses.find({}),
                models.leadTaskStatuses.find({}),
            ]);
            const sortFn = (a, b) => Number(a.position || 0) - Number(b.position || 0);
    ...
    router.post('/statuses/:kind', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            if (!repo)
                return res.status(400).json({ error: 'Invalid status kind' });
            const body = req.body || {};
            const label = validateLength(String(body.label || '').trim(), 2, 40, 'Label');
            const key = sanitizeStatusKey(body.key || body.label);
            if (!key)
                return res.status(400).json({ error: 'Status key is required' });
            ...
            if (existing)
                return res.status(409).json({ error: 'A status with this key already exists' });
    ...
    router.delete('/statuses/:kind/:id', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            if (status.is_system)
                return res.status(400).json({ error: 'System statuses cannot be deleted' });
    ...
    router.get('/sources', async (_req, res) => {
        try {
            const sources = await models.leadSources.find({});
    ...
    router.post('/sources', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            const existing = await models.leadSources.findOne({ key });
            if (existing)
                return res.status(409).json({ error: 'A source with this key already exists' });
            const all = await models.leadSources.find({});
            const position = all.reduce((max, s) => Math.max(max, Number(s.position || 0)), -1) + 1;
            const now = new Date().toISOString();
            const doc = {
    ...
    router.delete('/sources/:id', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            const id = String(req.params.id);
            const source = await models.leadSources.findById(id);
            if (!source)
                return res.status(404).json({ error: 'Source not found' });
            if (source.is_system)
                return res.status(400).json({ error: 'System sources cannot be deleted' });
    ...
    router.get('/', async (req, res) => {
        try {
            const user = req.user;
            const filter = (await buildLeadVisibilityFilter(models, user)) || {};
            const leads = await models.leads.find(filter);
    ...
    router.get('/tasks-list', async (req, res) => {
        try {
            ...
            const leads = await models.leads.find(visFilter || {});
            if (!leads.length)
                return res.json({ data: [], tasks: [] });
            const leadById = new Map(leads.map((l) => [String(l.id), l]));
            const tasks = await models.leadTasks.find({ lead_id: { $in: leads.map((l) => l.id) } });
            ...
            const users = assigneeIds.length
                ? await models.users.find({ id: { $in: assigneeIds } })
                : [];
    ...
    router.get('/:id', async (req, res) => {
        try {
            const user = req.user;
            const lead = await models.leads.findById(String(req.params.id));
            if (!lead)
    ...
    router.post('/', requireRole('admin', 'pm', 'pc', 'sales_manager', 'sales_tl'), async (req, res) => {
        try {
            ...
            const assignee = await models.users.findById(assignedTo);
            if (!assignee) {
    ...
    router.put('/:id', requireRole('admin', 'pm', 'pc', 'sales_manager', 'sales_tl'), async (req, res) => {
        try {
            const id = String(req.params.id);
            const body = req.body || {};
            const lead = await models.leads.findById(id);
            if (!lead)
                return res.status(404).json({ error: 'Lead not found' });
            ...
            if ('name' in body)
                patch.name = validateLength(String(body.name || '').trim(), 2, 120, 'Name');
            if ('email' in body)
                patch.email = validateEmail(body.email, 'Email');
            if ('phone' in body)
                patch.phone = validateLength(String(body.phone || '').trim(), 4, 30, 'Phone');
            if ('requirement' in body)
                patch.requirement = validateLength(String(body.requirement || '').trim(), 1, 5000, 'Requirement');
            ...
            if ('source' in body)
                patch.source = validateLength(String(body.source || '').trim(), 1, 80, 'Source');
            if ('notes' in body)
                patch.notes = String(body.notes || '').trim().slice(0, 5000);
            let nextStatus = null;
            if ('status' in body) {
            ...
            if ('assigned_to' in body) {
                ...
                if (String(lead.assigned_to) !== assignedTo)
                    reassignedToUser = assignee;
                // Cascade ownership change to any open follow-up task on this lead
                await models.leadTasks.updateMany({ lead_id: id, status: { $ne: 'done' } }, { $set: { assigned_to: assignedTo, updated_at: new Date().toISOString() } });
            }
    ...
    router.post('/:id/close', async (req, res) => {
        try {
            ...
            let projectExpectedEnd = null;
            let projectAmount = null;
            let projectDescription = null;
            let projectRemarks = null;
            let projectPriority = 'medium';
            let projectType = 'development';
            let projectStatus = 'active';
            let projectDeliveryKind = null;
            let projectBillable = 1;
            let projectPmId = null;
            let projectPcId = null;
            let projectSoldBy = null;
            let projectCommercialVisibleTo = [];
            ...
            if (projectInput) {
                ...
                if (Number.isNaN(sd.getTime()))
                    return res.status(400).json({ error: 'Invalid project start date' });
                ...
                if (projectInput.expected_end_date) {
                    ...
                    if (Number.isNaN(ed.getTime()))
                        return res.status(400).json({ error: 'Invalid project end date' });
                ...
                }
                {
                    ...
                    if (!Number.isFinite(amt) || amt <= 0)
                        return res.status(400).json({ error: 'Project Amount must be a positive number' });
                    projectAmount = amt;
                }
                ...
                if (projectInput.project_type) {
                    ...
                    if (!['development', 'maintenance', 'support', 'consulting'].includes(t))
                        return res.status(400).json({ error: 'Invalid project type' });
                    projectType = t;
                }
                if (projectInput.status) {
                    const s = String(projectInput.status).toLowerCase();
                    if (!['active', 'on_hold', 'completed', 'archived', 'cancelled'].includes(s))
                        return res.status(400).json({ error: 'Invalid project status' });
                    projectStatus = s;
                }
                if (projectInput.delivery_kind) {
                    const dk = String(projectInput.delivery_kind).toLowerCase();
                    if (!['app', 'web', 'both'].includes(dk))
                        return res.status(400).json({ error: 'Invalid delivery kind' });
                ...
                if (projectInput.pm_id) {
                    const pm = await models.users.findById(String(projectInput.pm_id));
                    if (!pm)
                        return res.status(400).json({ error: 'Selected PM not found' });
                    projectPmId = String(pm.id);
                }
                if (projectInput.pc_id) {
                    const pc = await models.users.findById(String(projectInput.pc_id));
                    if (!pc)
                        return res.status(400).json({ error: 'Selected PC not found' });
            ...
            if (projectInput) {
                ...
                const projectDoc = {
                    ...
                    sold_by: projectSoldBy || req.user?.name || req.user?.full_name || null,
            ...
            await models.leadTasks.updateMany({ lead_id: id, status: { $nin: ['done', 'skipped', 'cancelled'] } }, { $set: { status: 'done', updated_at: now } });
            await logLeadActivity(models, id, req.user, 'lead_closed', createdProject
                ? `Lead closed → client (${created.email}) + project "${createdProject.name}" (${createdProject.code}) created`
                : `Lead closed and converted to client (${created.email})`, { client_id: created.id, project_id: createdProject?.id || null, email: created.email });
            const rawLoginUrl = String(runtimeEnv.CLIENT_LOGIN_URL ||
            ...
            let mailResult = { sent: false, error: null };
            try {
    ...
    router.patch('/tasks/:taskId', async (req, res) => {
        try {
            ...
            const task = await models.leadTasks.findById(taskId);
            if (!task)
                return res.status(404).json({ error: 'Task not found' });
            const isOwner = String(task.assigned_to) === String(user?.sub);
            const isManager = LEADS_GLOBAL_ROLES.includes(role);
            ...
            if (!canEdit) {
                const lead = await models.leads.findById(String(task.lead_id));
            ...
            const patch = { updated_at: new Date().toISOString() };
            if ('status' in body)
                patch.status = String(body.status || 'pending').trim().toLowerCase();
            if ('notes' in body)
                patch.notes = String(body.notes || '').trim().slice(0, 2000);
            if ('due_date' in body)
                patch.due_date = body.due_date ? new Date(body.due_date).toISOString() : null;
            ...
            const summaryParts = [];
            if (patch.status)
                summaryParts.push(`status → ${patch.status}`);
            if (patch.due_date !== undefined)
                summaryParts.push(`due → ${patch.due_date || 'cleared'}`);
            if ('snooze_minutes' in patch)
                summaryParts.push(`snooze → ${patch.snooze_minutes}min`);
            if (summaryParts.length) {
                await logLeadActivity(models, String(task.lead_id), user, 'followup_updated', `Follow-up updated: ${summaryParts.join(', ')}`, { task_id: taskId, ...patch });
            }
    ...
    router.delete('/tasks/:taskId', async (req, res) => {
        try {
            ...
            await logLeadActivity(models, String(task.lead_id), user, kind, `${kind === 'task_deleted' ? 'Task' : 'Follow-up'} "${task.title || taskId}" deleted`, { task_id: taskId });
    ...
    router.post('/:id/followups', async (req, res) => {
        try {
            ...
            const lead = await models.leads.findById(leadId);
            if (!lead)
                return res.status(404).json({ error: 'Lead not found' });
            if (!(await canUserAccessLead(models, user, lead))) {
            ...
            await logLeadActivity(models, leadId, user, 'followup_added', `Scheduled follow-up "${title}" for ${dueDate.toISOString()}`, { task_id: taskId, snooze_minutes: snoozeMinutes });
    ...
    router.post('/tasks/:taskId/acknowledge', async (req, res) => {
        try {
            ...
            await logLeadActivity(models, String(task.lead_id), user, 'followup_acknowledged', `Follow-up alarm acknowledged`, { task_id: taskId });
    ...
    router.get('/followups/upcoming', async (req, res) => {
        try {
            ...
            const leads = await models.leads.find({ id: { $in: leadIds } });
            const leadById = new Map(leads.map((l) => [String(l.id), l]));
            const now = Date.now();
            const due = [];
            for (const t of tasks) {
    ...
    router.get('/:id/comments', async (req, res) => {
        try {
            ...
            const comments = await models.leadComments.find({ lead_id: leadId });
    ...
    router.post('/:id/comments', async (req, res) => {
        try {
            ...
            await logLeadActivity(models, leadId, user, 'comment_added', `Comment: ${text.length > 120 ? text.slice(0, 117) + '…' : text}`, { comment_id: id });
    ...
    router.get('/:id/timeline', async (req, res) => {
        try {
            ...
            const activities = await models.leadActivities.find({ lead_id: leadId });
            ...
            if (missingIds.length) {
                const users = await models.users.find({ id: { $in: [...new Set(missingIds)] } });
                const map = new Map(users.map((u) => [String(u.id), u.full_name]));
                for (const a of activities) {
                    if (!a.actor_name && a.actor_id)
                        a.actor_name = map.get(String(a.actor_id)) || null;
                }
            }
    ...
    async function sendLeadMail(req, res, kind) {
        const user = req.user;
        const leadId = String(req.params.id);
        const lead = await models.leads.findById(leadId);
        if (!lead)
        ...
        const finalHtml = html || `<pre style="font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap">${text.replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]))}</pre>`;
        let mailResult = { sent: false, error: null };
        try {
            const smtpHost = runtimeEnv.SMTP_HOST || (runtimeEnv.SMTP_USER || runtimeEnv.SENDER_EMAIL ? 'smtp.gmail.com' : '');
            const smtpUser = runtimeEnv.SMTP_USER || runtimeEnv.SENDER_EMAIL || '';
        ...
        await logLeadActivity(models, leadId, user, kind === 'portfolio' ? 'portfolio_sent' : 'mail_sent', mailResult.sent ? summary : `${summary} — failed: ${mailResult.error}`, {
            to, cc, subject,
    ...
    router.get('/:id/notes', async (req, res) => {
        try {
            ...
            const notes = await models.leadNotes.find({ lead_id: leadId });
    ...
    router.post('/:id/notes', async (req, res) => {
        try {
            ...
            await logLeadActivity(models, leadId, user, 'note_added', `Note: ${text.length > 120 ? text.slice(0, 117) + '…' : text}`, { note_id: id });
    ...
    router.post('/:id/tasks', async (req, res) => {
        try {
            ...
            await logLeadActivity(models, leadId, user, 'task_added', `Task added: "${title}" due ${dueDate.toISOString()}`, { task_id: taskId, priority });
    ...
    router.post('/import', requireRole('admin', 'pm', 'pc', 'sales_manager', 'sales_tl'), async (req, res) => {
        try {
            const user = req.user;
            const body = req.body || {};
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                try {
                    ...
                    await logLeadActivity(models, leadId, user, 'lead_created', `Lead imported and assigned to ${assignee.full_name || assignee.email}`, { assigned_to: assignedTo, source_import: true });
