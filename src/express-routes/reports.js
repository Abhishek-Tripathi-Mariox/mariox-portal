...
export function createReportsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/developer/:id', async (req, res) => {
        try {
            ...
            const [user, timesheets, assignments, projects] = await Promise.all([
                ...
                models.projectAssignments.find({ user_id: userId, is_active: 1 }),
    ...
    router.get('/project/:id', async (req, res) => {
        try {
            ...
            const [project, timesheets, assignments, users] = await Promise.all([
                ...
                models.projectAssignments.find({ project_id: projectId, is_active: 1 }),
                models.users.find({}),
            ]);
            if (!project)
    ...
    async function teamUtilization(monthStr, roles) {
        const { start, end } = monthRange(monthStr);
        const [users, timesheets, assignments] = await Promise.all([
            models.users.find({ role: { $in: roles }, is_active: 1 }),
            models.timesheets.find({
                date: { $gte: start, $lte: end },
                approval_status: { $ne: 'rejected' },
            }),
