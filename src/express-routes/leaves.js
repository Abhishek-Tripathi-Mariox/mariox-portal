...
import { validateEnum, validateISODate, validateLength, validateOptional, validateRange, respondWithError, } from '../validators';
import { createUserNotification, createUserNotifications } from './notifications';
const LEAVE_TYPES = ['casual', 'sick', 'earned', 'unpaid', 'maternity', 'paternity', 'wfh', 'other'];
const LEAVE_APPROVAL_STATUSES = ['approved', 'rejected', 'pending'];
export function createLeavesRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            if (targetUserId !== user.sub) {
                const actor = await models.users.findById(user.sub);
    ...
    router.patch('/:id/approve', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            const decisionReason = rawReason
                ...
                : null;
            const leave = await models.leaves.findById(String(req.params.id));
            if (!leave)
            ...
            const approver = await models.users.findById(user.sub);
