...
import { validateRequired, validateLength, validateISODate, validateOptional, respondWithError, } from '../validators';
import { sendInvoiceViaSmtp, parseEmailList, escapeHtml as esc } from '../utils/invoice-email';
export function createSprintsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const [sprints, tasks, users] = await Promise.all([
                models.sprints.find(filter),
                models.tasks.find({}),
                models.users.find({}),
            ]);
            const usersById = new Map(users.map((u) => [String(u.id), u]));
            const empty = () => ({ task_count: 0, done_count: 0, blocked_count: 0, total_story_points: 0, completed_story_points: 0 });
    ...
    router.post('/', async (req, res) => {
        try {
            const user = req.user;
            if (!(await userHasAnyPermission(models, user, 'sprints.create')))
    ...
    router.put('/:id', async (req, res) => {
        try {
            const user = req.user;
            if (!(await userHasAnyPermission(models, user, 'sprints.edit')))
                return res.status(403).json({ error: 'Forbidden' });
...
export function createMilestonesRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    function normalizeTasks(input) {
        if (!Array.isArray(input))
            return [];
        const now = new Date().toISOString();
        return input.slice(0, 50).map((t, idx) => {
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const [milestones, users, projects, invoices, allTasks] = await Promise.all([
                models.milestones.find(filter),
                models.users.find({}),
                models.projects.find({}),
                models.invoices.find({}),
                models.tasks.find({}),
            ]);
            const usersById = new Map(users.map((u) => [String(u.id), u]));
            ...
            const enriched = milestones.map((m) => {
                ...
                if (Array.isArray(m.tasks)) {
                    for (const ref of m.tasks)
                        refsByTaskId.set(String(ref.id), ref);
                }
    ...
    router.post('/', async (req, res) => {
        try {
            const user = req.user;
            if (!(await userHasAnyPermission(models, user, 'milestones.create')))
                return res.status(403).json({ error: 'Forbidden' });
            const body = req.body || {};
            ...
            if (!project_id || !title || !due_date)
                return res.status(400).json({ error: 'project_id, title, due_date required' });
            ...
            if (Array.isArray(attachments) && attachments.length) {
                try {
                    ...
                    if (docRows.length)
                        await models.documents.insertMany(docRows);
                }
                catch (e) {
    ...
    router.put('/:id', async (req, res) => {
        try {
            const user = req.user;
            if (!(await userHasAnyPermission(models, user, 'milestones.edit')))
                return res.status(403).json({ error: 'Forbidden' });
            const id = req.params.id;
            const body = req.body || {};
            const patch = { updated_at: new Date().toISOString() };
            ...
            if ('tasks' in body)
                patch.tasks = normalizeTasks(body.tasks);
    ...
    router.post('/:id/send-email', async (req, res) => {
        try {
            ...
            const project = milestone.project_id ? await models.projects.findById(String(milestone.project_id)) : null;
            const client = project?.client_id ? await models.clients.findById(String(project.client_id)) : null;
            const body = req.body || {};
            ...
            const liveTasks = await models.tasks.find({ milestone_id: id });
            const usersForEmail = await models.users.find({});
            const usersByIdEmail = new Map(usersForEmail.map((u) => [String(u.id), u]));
            const refsByIdEmail = new Map();
            if (Array.isArray(milestone.tasks)) {
                for (const ref of milestone.tasks)
                    refsByIdEmail.set(String(ref.id), ref);
            }
    ...
    router.post('/:id/rate', async (req, res) => {
        try {
            ...
            if (milestone.project_id) {
                const project = await models.projects.findById(String(milestone.project_id));
            ...
            if (!rating || rating.overall === 0)
                return res.status(400).json({ error: 'Provide at least one rating between 1 and 10' });
            if (!rating.rated_by) {
