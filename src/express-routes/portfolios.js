...
async function getPortfolioPerms(models, user) {
...
export function createPortfoliosRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    // ── LIST ─────────────────────────────────────────────────
    router.get('/', async (req, res) => {
        try {
            const portfolios = (await models.portfolios.find({}));
            ...
            const allSends = (await models.portfolioSends.find({}));
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const existing = (await models.portfolios.findOne({ id }));
            if (!existing)
            ...
            if ('title' in body)
                patch.title = validateLength(String(body.title || '').trim(), 2, 200, 'Title');
    ...
    router.get('/:id/history', async (req, res) => {
        try {
            const id = String(req.params.id);
            const sends = (await models.portfolioSends.find({ portfolio_id: id }));
    ...
    router.post('/:id/send/:leadId', async (req, res) => {
        try {
            ...
            const portfolio = (await models.portfolios.findOne({ id: portfolioId }));
            if (!portfolio)
                return res.status(404).json({ error: 'Portfolio not found' });
            ...
            const body = req.body || {};
            const subject = validateLength(String(body.subject || `Mariox Software — ${portfolio.title}`).trim(), 1, 300, 'Subject');
            ...
            if (portfolio.file?.url) {
                try {
                    ...
                    if (!fileRes.ok)
                        throw new Error(`Failed to fetch portfolio file (HTTP ${fileRes.status})`);
            ...
            const html = `<pre style="font-family:Arial,Helvetica,sans-serif;white-space:pre-wrap">${messageText.replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]))}</pre>`;
            let sentOk = false;
            let sendError = '';
            try {
            ...
            if (!sentOk)
                return res.status(502).json({ error: sendError || 'Failed to send portfolio' });
    ...
    router.get('/permissions', requireRole('admin'), async (_req, res) => {
        try {
            const grants = (await models.portfolioPermissions.find({}));
    ...
    router.post('/permissions', requireRole('admin'), async (req, res) => {
        try {
            ...
            const existing = (await models.portfolioPermissions.findOne({ user_id: userId }));
            if (existing)
