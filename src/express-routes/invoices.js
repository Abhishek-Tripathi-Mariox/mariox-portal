...
import { validateRequired, validateLength, validateOptional, validateISODate, validatePositiveNumber, validateRange, respondWithError, } from '../validators';
import { buildInvoiceEmailGST, sendInvoiceViaSmtp, parseEmailList, } from '../utils/invoice-email';
function sum(items, pick) {
    return items.reduce((total, item) => total + pick(item), 0);
}
function calcAmounts(amount, taxPct) {
...
export function createInvoicesRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const invoice = await models.invoices.findById(id);
            if (!invoice)
    ...
    router.put('/:id', requireRole('admin'), async (req, res) => {
        try {
            ...
            if ('description' in body)
                patch.description = body.description ? validateLength(String(body.description), 0, 2000, 'Description') : null;
            if ('status' in body) {
            ...
            if ('issue_date' in body && body.issue_date)
                patch.issue_date = validateISODate(body.issue_date, 'Issue date');
            if ('due_date' in body && body.due_date)
                patch.due_date = validateISODate(body.due_date, 'Due date');
            if ('paid_date' in body)
                patch.paid_date = body.paid_date ? validateISODate(body.paid_date, 'Paid date') : null;
            if ('paid_amount' in body)
                patch.paid_amount = validatePositiveNumber(body.paid_amount, 'Paid amount');
            if ('transaction_ref' in body)
                patch.transaction_ref = body.transaction_ref || null;
            if ('file_url' in body)
                patch.file_url = body.file_url || null;
            if ('notes' in body)
                patch.notes = body.notes || null;
            if ('payment_terms' in body)
                patch.payment_terms = body.payment_terms || null;
