...
import { validateEmail, validateNewPassword, validateName, validateOptional, validatePhone, validateLength, validateHexColor, respondWithError, } from '../validators';
...
async function maskPrice(models, user, record) {
    if (!record)
        return record;
    ...
    if (ok)
        return record;
    const { price: _omit, ...rest } = record;
    return rest;
}
export function createClientsRouter(models, jwtSecret, passwordSalt = '') {
    const router = Router();
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            if ('company_name' in body)
                patch.company_name = validateName(body.company_name, 'Company name', 2, 120);
            if ('contact_name' in body)
                patch.contact_name = validateName(body.contact_name, 'Contact name', 2, 100);
            if ('phone' in body)
                patch.phone = validateOptional(body.phone, (v) => validatePhone(v, 'Phone'));
            if ('website' in body)
                patch.website = body.website ? String(body.website).trim().slice(0, 200) : null;
            if ('industry' in body)
                patch.industry = validateOptional(body.industry, (v) => validateLength(String(v).trim(), 2, 80, 'Industry'));
            ...
            if ('state_code' in body)
                patch.state_code = body.state_code ? String(body.state_code).trim().toUpperCase().slice(0, 8) : null;
            if ('pincode' in body) {
            ...
            if ('price' in body) {
                ...
                if (!canEditPrice)
                    return res.status(403).json({ error: 'Forbidden: cannot edit price' });
            ...
            if ('country' in body)
                patch.country = body.country ? String(body.country).trim().slice(0, 80) : null;
            if ('avatar_color' in body && body.avatar_color)
                patch.avatar_color = validateHexColor(body.avatar_color, 'Avatar color');
    ...
    router.post('/:id/reset-password', async (req, res) => {
        try {
            ...
            const target = await models.clients.findById(id);
            if (!target)
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            if (!client)
                return res.status(404).json({ error: 'Client not found' });
            const projects = (await models.projects.find({ client_id: id })).map((project) => ({
                ...project,
                pm_name: undefined,
                team_size: 0,
                task_count: 0,
            ...
            const invoices = await models.invoices.find({ client_id: id });
            const notifications = await models.notifications.find({ client_id: id });
            const maskedClient = await maskPrice(models, user, client);
    ...
    router.get('/:id/dashboard', async (req, res) => {
        try {
            ...
            if (!client)
                return res.status(404).json({ error: 'Client not found' });
            const projects = await models.projects.find({ client_id: id });
            const billingInvoices = await models.invoices.find({ client_id: id });
            const milestones = await models.milestones.find({});
            const notifications = await models.notifications.find({ client_id: id });
            const updates = await models.activityLogs.find({ project_id: { $in: projects.map((p) => p.id) } });
            return res.json({
                projects,
                billing: {
        ...
        try {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                try {
                    ...
                    created.push({ id: cl?.id, email, company_name: company });
                }
                catch (e) {
