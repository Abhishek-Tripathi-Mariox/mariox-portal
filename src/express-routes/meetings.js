...
async function getMeetingPerms(models, user) {
...
function normalizeMeetingPayload(body) {
...
function buildInviteHtml(meeting, lead, senderName, opts = {}) {
...
function buildInviteText(meeting, lead, senderName, opts = {}) {
    ...
    if (meeting.meeting_link)
        lines.push(`Join: ${meeting.meeting_link}`);
    if (meeting.agenda) {
...
function buildAttendeeHtml(meeting, lead, attendeeName, senderName, opts = {}) {
    const esc = (s) => String(s || '').replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
    const when = new Date(meeting.scheduled_at).toLocaleString('en-IN', { dateStyle: 'full', timeStyle: 'short' });
    const linkBlock = meeting.meeting_link
...
function buildAttendeeText(meeting, lead, attendeeName, senderName, opts = {}) {
    const when = new Date(meeting.scheduled_at).toLocaleString('en-IN', { dateStyle: 'full', timeStyle: 'short' });
    const intro = opts.isReschedule
...
async function sendMeetingInvites(models, runtimeEnv, meeting, lead, senderName, options = {}) {
    ...
    const attendeeIds = (Array.isArray(meeting.attendees) ? meeting.attendees : [])
        .map((x) => String(x || ''))
        .filter((x) => x && x !== options.excludeUserId);
    if (attendeeIds.length) {
        const users = (await models.users.find({ id: { $in: attendeeIds } }));
...
// ───────────────────────────────────────────────────────────────
export function startMeetingReminderTick(models, intervalMs = 60_000) {
    ...
    async function tick() {
        ...
        try {
            ...
            for (const m of startedMeetings) {
                ...
                if (endMs > now)
                    continue;
                const claim = await models.meetings.updateOne({ id: m.id, status: 'scheduled' }, { $set: { status: 'completed', auto_completed_at: nowIso, updated_at: nowIso } });
            ...
            for (const m of candidates) {
                ...
                const claim = await models.meetings.updateOne({ id: m.id, reminder_sent: { $ne: 1 } }, { $set: { reminder_sent: 1, reminder_sent_at: new Date().toISOString() } });
                const claimed = (claim && (claim.modifiedCount || claim.matchedCount || claim.result?.nModified)) || 0;
                if (!claimed)
                    continue;
                const attendees = Array.isArray(m.attendees) ? m.attendees : [];
                const recipients = new Set(attendees.map((id) => String(id || '')).filter(Boolean));
                if (m.created_by)
                    recipients.add(String(m.created_by));
...
export function createMeetingsRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const meetings = (await models.meetings.find(filter));
            ...
            for (const m of meetings) {
                if (m.created_by)
                    userIds.add(String(m.created_by));
                for (const a of (m.attendees || []))
                    userIds.add(String(a));
            }
            ...
            const enriched = meetings.map((m) => ({
                ...
                attendee_details: (m.attendees || []).map((aid) => ({
                    id: aid,
    ...
    router.get('/:id', async (req, res) => {
        try {
            ...
            const meeting = (await models.meetings.findOne({ id }));
            if (!meeting)
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const lead = (await models.leads.findOne({ id: payload.lead_id }));
            if (!lead)
            ...
            const recipients = new Set(payload.attendees.map(String).filter(Boolean));
            if (user?.sub)
                recipients.add(String(user.sub));
            await createUserNotifications(models, Array.from(recipients), {
            ...
            if (emailResult.sentTo.length) {
                await models.meetings.updateOne({ id }, { $set: { invite_sent_at: new Date().toISOString(), invite_sent_to: emailResult.sentTo } });
                try {
                    await models.leadActivities.insertOne({
                        id: generateId('lact'),
                        lead_id: payload.lead_id,
    ...
    router.put('/:id', async (req, res) => {
        try {
            const user = req.user;
            const id = String(req.params.id);
            const existing = (await models.meetings.findOne({ id }));
            if (!existing)
                return res.status(404).json({ error: 'Meeting not found' });
    ...
    router.post('/:id/reschedule', async (req, res) => {
        try {
            ...
            if (newWhen !== existing.scheduled_at)
                patch.reminder_sent = 0;
            await models.meetings.updateOne({ id }, { $set: patch });
            // Re-send invite emails with "Rescheduled" framing.
            const lead = (await models.leads.findOne({ id: existing.lead_id }));
            let invites = { sent: 0, failed: 0, skipped: false };
            if (lead) {
                const senderName = user?.full_name || user?.name || 'Mariox Team';
            ...
            const recipients = new Set((existing.attendees || []).map(String).filter(Boolean));
            if (existing.created_by)
                recipients.add(String(existing.created_by));
    ...
    router.post('/:id/send-invite', async (req, res) => {
        try {
            ...
            if (!meeting)
                return res.status(404).json({ error: 'Meeting not found' });
            const lead = (await models.leads.findOne({ id: meeting.lead_id }));
            if (!lead)
                return res.status(404).json({ error: 'Lead not found' });
            const senderName = user?.full_name || user?.name || 'Mariox Team';
            ...
            if (result.sentTo.length) {
                await models.meetings.updateOne({ id }, { $set: { invite_sent_at: sentAt, invite_sent_to: result.sentTo } });
                try {
                    await models.leadActivities.insertOne({
                        id: generateId('lact'),
