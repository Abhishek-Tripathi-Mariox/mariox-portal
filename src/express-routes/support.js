...
import { TICKET_STATUS, TICKET_PRIORITY, TICKET_CATEGORIES, } from '../constants';
import { validateLength, validateEnum, respondWithError, } from '../validators';
...
const VALID_CATEGORY = new Set(TICKET_CATEGORIES);
function role(user) {
...
function canViewTicket(user, ticket, scope) {
    if (!ticket)
        return false;
    if (isPmOrAdmin(user))
        return true;
    ...
    if (isDeveloper(user)) {
        if (ticket.assigned_to_id === user.sub)
            return true;
        if (ticket.created_by_id === user.sub)
            return true;
        if (ticket.project_id && scope.projectIds.has(String(ticket.project_id)))
            return true;
        if (ticket.project_id && scope.assignedProjectIds.has(String(ticket.project_id)))
            return true;
        return false;
    }
    return false;
}
async function loadUserProjectScope(models, userId) {
    const [memberships, assignments] = await Promise.all([
        models.projectTeamMembers.find({ user_id: userId }),
        models.projectAssignments.find({ user_id: userId }),
    ]);
    const teamIds = memberships.map((m) => m.project_team_id || m.team_id).filter(Boolean);
    const teamProjectIds = new Set();
    if (teamIds.length) {
        const teams = await models.projectTeams.find({ id: { $in: teamIds } });
        for (const t of teams)
            if (t.project_id)
                teamProjectIds.add(String(t.project_id));
        ...
        for (const p of externalProjects)
            teamProjectIds.add(String(p.id));
    }
    ...
    for (const p of externalUserProjects)
        teamProjectIds.add(String(p.id));
    const assignedProjectIds = new Set(assignments.map((a) => String(a.project_id)).filter(Boolean));
    return { projectIds: teamProjectIds, assignedProjectIds };
}
async function loadEligibleAssigneeIds(models, projectId) {
    const ids = new Set();
    if (!projectId) {
    ...
    if (!project)
        return ids;
    if (project.pm_id)
        ids.add(String(project.pm_id));
    if (project.team_lead_id)
        ids.add(String(project.team_lead_id));
    if (project.assignment_type === 'external' && project.external_team_id) {
        ...
        const assignments = await models.projectAssignments.find({ project_id: projectId });
...
async function resolveActorName(models, actorId, actorRole) {
    if (!actorId)
        return 'System';
    if (actorRole === 'client') {
        const c = await models.clients.findById(actorId);
        return c?.contact_name || c?.company_name || c?.email || 'Client';
    }
    const u = await models.users.findById(actorId);
    return u?.full_name || u?.email || 'Staff';
}
async function resolveUserName(models, userId) {
    if (!userId)
        return null;
    const u = await models.users.findById(userId);
    if (u)
        return u.full_name || u.email || null;
    const c = await models.clients.findById(userId);
 ...
 */
async function ticketStakeholderIds(models, ticket) {
    const ids = new Set();
    if (ticket.created_by_id)
        ids.add(String(ticket.created_by_id));
    if (ticket.assigned_to_id)
        ids.add(String(ticket.assigned_to_id));
    if (ticket.project_id) {
        const project = await models.projects.findById(ticket.project_id);
        if (project?.pm_id)
            ids.add(String(project.pm_id));
        if (project?.pc_id)
            ids.add(String(project.pc_id));
    }
    return Array.from(ids);
}
async function recordTicketEvent(models, params) {
    try {
...
export function createSupportRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/tickets', async (req, res) => {
        try {
            const user = req.user;
            const { status, priority, project_id, assigned_to_id, client_id } = req.query;
            const filter = {};
            if (status)
            ...
            if (isPmOrAdmin(user)) {
                ...
                const visibility = [{ assigned_to_id: user.sub }, { created_by_id: user.sub }];
                if (allProjectIds.length)
                    visibility.push({ project_id: { $in: allProjectIds } });
            ...
            for (const t of tickets) {
                if (t.assigned_to_id)
                    userIds.add(t.assigned_to_id);
                if (t.created_by_role !== 'client' && t.created_by_id)
                    userIds.add(t.created_by_id);
                if (t.client_id)
                    clientIds.add(t.client_id);
                if (t.project_id)
                    projectIds.add(t.project_id);
            }
            const [users, clients, projects] = await Promise.all([
                userIds.size ? models.users.find({ id: { $in: Array.from(userIds) } }) : Promise.resolve([]),
                clientIds.size ? models.clients.find({ id: { $in: Array.from(clientIds) } }) : Promise.resolve([]),
                projectIds.size ? models.projects.find({ id: { $in: Array.from(projectIds) } }) : Promise.resolve([]),
            ]);
            ...
            const enriched = tickets.map((t) => ({
                ...
                created_by_name: t.created_by_role === 'client'
    ...
    router.post('/tickets', async (req, res) => {
        try {
            ...
            const priority = validateEnum(body.priority || TICKET_PRIORITY.MEDIUM, Object.values(TICKET_PRIORITY), 'Priority');
            const category = validateEnum(body.category || 'other', TICKET_CATEGORIES, 'Category');
            let clientId = null;
    ...
    router.get('/tickets/:id', async (req, res) => {
        try {
            ...
            const scope = isDeveloper(user)
                ...
                : { projectIds: new Set(), assignedProjectIds: new Set() };
            if (!canViewTicket(user, ticket, scope)) {
                return res.status(403).json({ error: 'Forbidden' });
            }
            const commentFilter = { ticket_id: ticket.id };
            if (isClient(user))
                commentFilter.is_internal = { $ne: 1 };
            const comments = await models.supportComments.find(commentFilter, { sort: { created_at: 1 } });
            const events = await models.supportEvents.find({ ticket_id: ticket.id }, { sort: { created_at: 1 } });
        ...
        try {
            ...
            if (isPmOrAdmin(user)) {
                ...
                if ('status' in body) {
                    ...
                    patch.status = next;
                    if (next === TICKET_STATUS.RESOLVED && !ticket.resolved_at)
                        patch.resolved_at = now;
                    if (next === TICKET_STATUS.CLOSED && !ticket.closed_at)
                        patch.closed_at = now;
                }
            }
            else if (isDeveloper(user)) {
            ...
            for (const f of trackedFields) {
                if (!(f.key in patch))
                    continue;
                const before = ticket[f.key] ?? null;
                const after = patch[f.key] ?? null;
                if (String(before || '') === String(after || ''))
                    continue;
                let fromLabel = before ? String(before) : null;
                let toLabel = after ? String(after) : null;
                if (f.resolveLabels) {
                    if (f.key === 'assigned_to_id') {
                        ...
                        const proj = after ? await models.projects.findById(after) : null;
                        const old = before ? await models.projects.findById(before) : null;
    ...
    router.post('/tickets/:id/comments', async (req, res) => {
        try {
            ...
            const body = req.body || {};
            const text = validateLength(String(body.body || body.message || '').trim(), 1, 5000, 'Comment body');
            ...
            if (isInternal)
                recipients = recipients.filter((id) => id !== ticket.client_id);
    ...
    router.get('/assignees', async (req, res) => {
        try {
            ...
            if (isClient(user)) {
                ...
                if (!projectId)
                    return res.json({ assignees: [], groups: {} });
                const project = await models.projects.findById(projectId);
                if (!project || project.client_id !== user.sub) {
                    return res.status(403).json({ error: 'Forbidden' });
                }
                ...
                const users = ids.length
                    ? await models.users.find({ id: { $in: ids } })
                    : [];
            ...
            const groups = { managers: [], developers: [], team: [] };
            if (!projectId) {
            ...
            if (managerIds.length) {
                groups.managers = await models.users.find({ id: { $in: managerIds } });
            }
            if (project.assignment_type === 'external' && project.external_team_id) {
                if (project.external_assignee_type === 'user') {
                    const teamUser = await models.users.findById(project.external_team_id);
                    if (teamUser)
                        groups.team = [teamUser];
                }
                else {
                    const members = await models.projectTeamMembers.find({
                        project_team_id: project.external_team_id,
                    ...
                    if (memberUserIds.length) {
                        groups.team = await models.users.find({ id: { $in: memberUserIds } });
                    }
                }
            }
            else {
                ...
                if (devIds.length) {
                    groups.developers = await models.users.find({ id: { $in: devIds } });
                }
            }
    ...
    router.get('/stats', async (req, res) => {
        try {
            ...
            if (!isPmOrAdmin(user))
                return res.status(403).json({ error: 'Forbidden' });
            const tickets = await models.supportTickets.find({});
            const byStatus = {};
            const byPriority = {};
            for (const t of tickets) {
