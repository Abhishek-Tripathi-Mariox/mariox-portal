...
import { extractS3Key, getS3Object } from '../utils/s3-upload';
import { validateRequired, validateLength, validateEnum, validateOptional, validateUrl, validatePositiveNumber, respondWithError, } from '../validators';
const DOC_VISIBILITIES = ['internal', 'client', 'all'];
export function createDocumentsRouter(models, jwtSecret, runtimeEnv = {}) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    router.get('/', async (req, res) => {
        try {
            const user = req.user;
            const { project_id, category, visibility } = req.query;
            const filter = {};
            if (project_id)
            ...
            if (role === 'client') {
                const myProjects = await models.projects.find({ client_id: user.sub });
            ...
            if (role !== 'client') {
                for (const a of auctions || []) {
                    ...
                    if (!atts.length)
                        continue;
                    if (project_id && String(a.resulting_project_id || '') !== String(project_id))
                        continue;
                    if (category && category !== 'bid')
                        continue;
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            if (isClient) {
                ...
                if (project_id) {
                    const project = await models.projects.findById(project_id);
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const doc = await models.documents.findById(id);
            if (!doc)
            ...
            for (const k of editableFields) {
                if (k in body)
                    patch[k] = k === 'tags' && Array.isArray(body[k]) ? JSON.stringify(body[k]) : body[k];
            }
    ...
    router.get('/:id/preview', async (req, res) => {
        try {
            ...
            if (!key)
                return res.status(400).json({ error: 'File location not available' });
