...
import { validateEmail, validateNewPassword, validateRequired, validateName, validatePhone, validateOptional, validateLength, respondWithError, } from '../validators';
...
export function createClientAuthRouter(models, jwtSecret, passwordSalt) {
    ...
    router.post('/login', async (req, res) => {
        try {
            ...
            const client = await models.clients.findActiveByEmail(email);
            if (!client)
    ...
    router.post('/impersonate/:id', createAuthMiddleware(jwtSecret), requireRole('admin'), async (req, res) => {
        try {
            ...
            const payload = {
                ...
                name: client.contact_name,
                company: client.company_name,
                impersonated_by: req.user?.sub || null,
    ...
    router.get('/me', async (req, res) => {
        try {
            ...
            const payload = (await jwtVerify(authHeader.slice(7), encoder.encode(jwtSecret))).payload;
            const client = await models.clients.findById(payload.sub, { projection: { id: 1, email: 1, company_name: 1, contact_name: 1, phone: 1, website: 1, industry: 1, avatar_color: 1 } });
            if (!client)
