...
const DOC_TYPES = ['offer_letter', 'experience_certificate', 'salary_certificate', 'appointment_letter', 'relieving_letter'];
export function createHrDocumentsRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const [rows, users] = await Promise.all([
                models.hrDocuments.find(filter),
                models.users.find({}),
            ]);
