...
import { validateRequired, validateLength, validateOptional, validateISODate, validateRange, validateEnum, respondWithError, } from '../validators';
function sum(items, pick) {
    return items.reduce((total, item) => total + pick(item), 0);
}
async function recomputeConsumedHours(models, projectId, userId) {
    ...
    await models.projectAssignments.updateMany({ project_id: projectId, user_id: userId, is_active: 1 }, { $set: { consumed_hours: total, updated_at: new Date().toISOString() } });
...
const TS_STATUSES = ['in_progress', 'completed', 'blocked', 'review'];
const TS_APPROVAL_ACTIONS = ['approve', 'approved', 'reject', 'rejected'];
export function createTimesheetsRouter(models, jwtSecret) {
    const router = Router();
    router.use(createAuthMiddleware(jwtSecret));
    router.get('/', async (req, res) => {
        try {
            const user = req.user;
            const { user_id, project_id, date_from, from, date_to, to, approval_status, date } = req.query;
            const start = date_from || from;
            const end = date_to || to;
            const timesheets = await models.timesheets.find({});
            ...
            if (user.role === 'developer')
                ...
                rows = rows.filter((row) => row.user_id === user_id);
            if (project_id)
                rows = rows.filter((row) => row.project_id === project_id);
            if (date)
                rows = rows.filter((row) => row.date === date);
            if (start)
                rows = rows.filter((row) => String(row.date || '') >= start);
            if (end)
                rows = rows.filter((row) => String(row.date || '') <= end);
            if (approval_status)
                rows = rows.filter((row) => row.approval_status === approval_status);
    ...
    router.get('/summary/weekly', async (req, res) => {
        try {
            ...
            const timesheets = await models.timesheets.find({ user_id: effectiveUserId, approval_status: { $ne: 'rejected' } });
            const projects = await models.projects.find({});
    ...
    router.get('/suggestions', async (req, res) => {
        try {
            const user = req.user;
            const timesheets = await models.timesheets.find({ user_id: user.sub });
            const projects = await models.projects.find({});
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const entry = await models.timesheets.findById(id);
            if (!entry)
            ...
            if ('hours_consumed' in body)
                patch.hours_consumed = validateRange(body.hours_consumed, 0, 24, 'Hours');
            if ('task_description' in body)
                patch.task_description = validateLength(String(body.task_description || '').trim(), 1, 1000, 'Task description');
            if ('module_name' in body)
                patch.module_name = validateOptional(body.module_name, (v) => validateLength(String(v).trim(), 1, 200, 'Module'));
            if ('is_billable' in body)
                patch.is_billable = body.is_billable ? 1 : 0;
            if ('extra_hours_reason' in body)
                patch.extra_hours_reason = body.extra_hours_reason || null;
            if ('status' in body)
                patch.status = validateEnum(body.status, TS_STATUSES, 'Status');
            if ('blocker_remarks' in body)
                patch.blocker_remarks = body.blocker_remarks || null;
    ...
    router.post('/bulk-approve', requireRole('admin', 'pm', 'pc'), async (req, res) => {
        try {
            ...
            const pmNotes = req.body?.pm_notes ? String(req.body.pm_notes).slice(0, 1000) : null;
            const now = new Date().toISOString();
            const entries = await models.timesheets.find({ id: { $in: ids } });
            await models.timesheets.updateMany({ id: { $in: ids } }, { $set: { approval_status: newStatus, approved_by: user.sub, approved_at: now, pm_notes: pmNotes, updated_at: now } });
