...
function toPct(consumed, total) {
    return total > 0 ? Math.round((consumed / total) * 100) : 0;
}
export function createDashboardRouter(models, jwtSecret) {
    const router = Router();
    ...
    router.get('/pm', async (req, res) => {
        try {
            ...
            const assignments = await models.projectAssignments.find({});
            ...
            for (const entry of timesheets) {
                ...
                if (Number(entry.is_billable || 0) === 1)
                    current.billable_hours += Number(entry.hours_consumed || 0);
    ...
    router.get('/developer', async (req, res) => {
        try {
            ...
            const projects = await models.projects.find({});
            const assignments = await models.projectAssignments.find({ user_id: userId, is_active: 1 });
            const timesheets = await models.timesheets.find({ user_id: userId });
