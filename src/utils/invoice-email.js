...
function isSameStateGST(env, place) {
...
function resolveSignature(env) {
    const explicit = String(env.INVOICE_SIGNATURE_URL || '').trim();
    if (explicit)
        return { src: explicit, attachment: null };
...
export function buildInvoiceEmailGST({ inv, client, project, env }) {
    ...
    const inlineImages = signature.attachment ? [signature.attachment] : [];
...
function buildMime(opts) {
    ...
    for (const img of inline) {
        parts.push(`--${relBoundary}`, `Content-Type: ${img.contentType}; name="${img.filename}"`, 'Content-Transfer-Encoding: base64', `Content-ID: <${img.cid}>`, `Content-Disposition: inline; filename="${img.filename}"`, '', wrapBase64(img.base64));
    }
...
export async function sendInvoiceViaSmtp(opts) {
    ...
    if (!host || !user || !pass)
        throw new Error('SMTP is not configured (need SMTP_HOST, SMTP_USER and SMTP_PASS)');
    ...
    const cmd = async (line, allowed, action) => {
        await send(line);
    ...
    try {
        ...
        if (reply.code !== 220)
            throw new Error(`SMTP greeting failed: ${reply.lines.join(' | ')}`);
        ...
        if (!secure) {
            ...
            const sec = tls.connect({ socket: socket, servername: host });
            await new Promise((resolve, reject) => {
        ...
        if (reply.code !== 250)
            throw new Error(`Message delivery failed: ${reply.lines.join(' | ')}`);
        try {
            await send('QUIT');
            await readReply();
        }
