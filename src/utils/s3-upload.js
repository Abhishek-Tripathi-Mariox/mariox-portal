...
function extFromMime(mime, fallbackExt) {
    ...
    if (m.startsWith('video/mp4'))
        return 'mp4';
    if (m.startsWith('video/quicktime'))
        return 'mov';
...
export async function uploadFileToS3(env, opts) {
...
export function extractS3Key(env, fileUrl) {
    if (!fileUrl)
        return null;
    try {
...
export async function getS3Object(env, key) {
    const { client, bucket } = getS3Client(env);
    const result = await client.send(new GetObjectCommand({ Bucket: bucket, Key: key }));
    return {
        body: result.Body, // Readable stream in Node
