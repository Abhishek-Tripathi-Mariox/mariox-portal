...
async function ensureAdminUser(models, passwordSalt, config) {
    const existing = await models.users.findByEmail(config.email);
    if (existing)
        return existing;
...
export async function bootstrapSeed(models, runtimeEnv) {
...
async function ensureLeadStatuses(models) {
    const now = new Date().toISOString();
...
async function ensureLeadSources(models) {
    const now = new Date().toISOString();
...
async function ensureSystemRoles(models) {
    ...
    for (const seed of SYSTEM_ROLE_SEEDS) {
        const existing = await models.roles.findOne({ key: seed.key });
        if (existing) {
            ...
            const patch = { is_system: 1, updated_at: now };
            if (!existing.name)
                patch.name = seed.name;
            if (!existing.description)
                patch.description = seed.description;
            if (!Array.isArray(existing.permissions))
                patch.permissions = seed.permissions;
            if (existing.key === 'pc' && Array.isArray(existing.permissions)) {
                patch.permissions = Array.from(new Set(existing.permissions.map((p) => String(p)).filter((p) => p && p !== 'projects.view_all')));
            }
