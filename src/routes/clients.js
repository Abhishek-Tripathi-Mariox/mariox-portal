...
clients.post('/', async (c) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1)
    `).bind(id, email, password_hash, company_name, contact_name, body.phone || null, body.website || null, body.industry || null, avatar_color).run();
...
clients.get('/:id', async (c) => {
    try {
        ...
        if (user.role === 'client' && user.sub !== id)
            return c.json({ error: 'Forbidden' }, 403);
...
clients.post('/:id/project/:project_id/comment', async (c) => {
    try {
        ...
        const client = await c.env.DB.prepare('SELECT contact_name, company_name FROM clients WHERE id=?').bind(id).first();
...
clients.put('/:id', async (c) => {
    try {
        const user = c.get('user');
        if (user.role === 'client' && user.sub !== c.req.param('id'))
            return c.json({ error: 'Forbidden' }, 403);
        const id = c.req.param('id');
        const body = await c.req.json();
        const fields = [];
