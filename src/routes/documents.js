...
documents.get('/', async (c) => {
    ...
    `;
        ...
        if (user.role === 'client') {
            sql += ' AND (d.visibility=? OR d.visibility=?)';
            params.push('client', 'all');
        }
...
documents.post('/', async (c) => {
    try {
        ...
        if (!project_id || !title || !file_name || !file_url)
            return c.json({ error: 'project_id, title, file_name, file_url required' }, 400);
