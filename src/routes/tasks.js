...
async function ensureColumnsExist(db, project_id) {
    const existing = await db.prepare('SELECT COUNT(*) as cnt FROM kanban_columns WHERE project_id=?').bind(project_id).first();
    if (existing?.cnt > 0)
        return;
    // Seed defaults
...
tasks.get('/columns/:project_id', async (c) => {
    try {
        ...
        const cols = await c.env.DB.prepare('SELECT * FROM kanban_columns WHERE project_id=? ORDER BY position ASC').bind(project_id).all();
...
tasks.post('/columns/:project_id', async (c) => {
    try {
        ...
        const maxPos = await c.env.DB.prepare('SELECT MAX(position) as m FROM kanban_columns WHERE project_id=?').bind(project_id).first();
...
tasks.get('/board/:project_id', async (c) => {
    try {
        ...
        const colResult = await c.env.DB.prepare('SELECT * FROM kanban_columns WHERE project_id=? ORDER BY position ASC').bind(project_id).all();
        const columnDefs = colResult.results;
        let sql = `
      SELECT t.*,
...
tasks.post('/', async (c) => {
    try {
        ...
        if (!perm.allowed)
            return c.json({ error: 'You do not have permission to create tasks on this board', reason: perm.reason }, 403);
...
tasks.put('/:id', async (c) => {
    try {
        const user = c.get('user');
        const id = c.req.param('id');
        const old_task = await c.env.DB.prepare('SELECT * FROM tasks WHERE id=?').bind(id).first();
        if (!old_task)
            return c.json({ error: 'Task not found' }, 404);
        ...
        if (!perm.allowed)
            return c.json({ error: 'You do not have permission to edit this task', reason: perm.reason }, 403);
        const body = await c.req.json();
        const fields = [];
        const vals = [];
...
tasks.delete('/:id', async (c) => {
    try {
        const user = c.get('user');
        const id = c.req.param('id');
        const task = await c.env.DB.prepare('SELECT project_id FROM tasks WHERE id=?').bind(id).first();
        if (!task)
            return c.json({ error: 'Task not found' }, 404);
        const perm = await checkKanbanPerm(c.env.DB, task.project_id, user.role, user.sub, 'can_delete_task');
        if (!perm.allowed)
            return c.json({ error: 'You do not have permission to delete tasks', reason: perm.reason }, 403);
...
tasks.post('/:id/comment', async (c) => {
    try {
        ...
        const task = await c.env.DB.prepare('SELECT project_id FROM tasks WHERE id=?').bind(task_id).first();
        if (!task)
            return c.json({ error: 'Task not found' }, 404);
        const perm = await checkKanbanPerm(c.env.DB, task.project_id, user.role, user.sub, 'can_comment');
        if (!perm.allowed)
            return c.json({ error: 'You do not have permission to comment on this board' }, 403);
...
tasks.patch('/:id/move', async (c) => {
    try {
        ...
        const old = await c.env.DB.prepare('SELECT status, project_id, assignee_id FROM tasks WHERE id=?').bind(id).first();
        if (!old)
            return c.json({ error: 'Task not found' }, 404);
        const perm = await checkKanbanPerm(c.env.DB, old.project_id, user.role, user.sub, 'can_move_task');
        if (!perm.allowed)
            return c.json({ error: 'You do not have permission to move tasks', reason: perm.reason }, 403);
