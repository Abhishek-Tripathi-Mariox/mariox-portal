...
clientAuth.post('/login', async (c) => {
    try {
        ...
        const client = await c.env.MODELS.clients.findActiveByEmail(email);
        if (!client)
...
clientAuth.get('/me', async (c) => {
    try {
        ...
        const client = await c.env.MODELS.clients.findById(payload.sub, { projection: { id: 1, email: 1, company_name: 1, contact_name: 1, phone: 1, website: 1, industry: 1, avatar_color: 1 } });
        if (!client)
