...
const projectTeams = new Hono();
projectTeams.use('*', authMiddleware);
async function createTeam(c, projectId, body) {
...
projectTeams.get('/project/:projectId', async (c) => {
    ...
    `).bind(projectId).all();
        ...
        const results = await Promise.all(teams.results.map(async (t) => {
...
projectTeams.put('/:teamId', requireRole('admin', 'pm'), async (c) => {
    try {
      ...
      WHERE id=?
    `).bind(body.name, body.description || null, body.team_lead_id || null, body.color || '#6366f1', teamId).run();
