...
function formatInvoiceCurrency(amount, currency = 'INR') {
...
function fromEmailFallback(inv) {
...
async function readSmtpReply(reader) {
...
async function sendInvoiceViaSmtp(env, invoice, to, cc, subject, html, text) {
    const brandName = 'Mariox Software Pvt Ltd';
    ...
    try {
        ...
        if (!secure) {
            ...
            const secureSocket = tls.connect({ socket: socket, servername: host });
            await new Promise((resolve, reject) => {
...
invoices.get('/', async (c) => {
    try {
        ...
        const result = await c.env.DB.prepare(invoiceQuery + where + `
      ORDER BY datetime(i.created_at) DESC, datetime(i.issue_date) DESC, i.id DESC
      LIMIT ? OFFSET ?
    `).bind(...params, limit, offset).all();
        const totalRow = await c.env.DB.prepare(`SELECT COUNT(*) as total FROM invoices i${where}`).bind(...params).first();
        // Summary stats
...
invoices.post('/:id/send-email', async (c) => {
    try {
        const user = c.get('user');
        if (user.role !== 'admin')
            return c.json({ error: 'Only Super Admin can send invoices' }, 403);
        const id = c.req.param('id');
        const body = await c.req.json().catch(() => ({}));
        const invoice = await c.env.DB.prepare(invoiceQuery + ' WHERE i.id=?').bind(id).first();
        if (!invoice)
            return c.json({ error: 'Invoice not found' }, 404);
        const to = parseEmailList(body.to || invoice.client_email);
        const cc = parseEmailList(body.cc);
        if (!to.length)
            return c.json({ error: 'Client email is required' }, 400);
        ...
        await c.env.DB.prepare(`INSERT INTO activity_logs (id,project_id,entity_type,entity_id,action,actor_user_id,actor_name,actor_role,new_value,metadata) VALUES (?,?,?,?,?,?,?,?,?,?)`)
            .bind('al-' + Date.now(), invoice.project_id, 'invoice', id, 'status_changed', user.sub, user.name, user.role, `Invoice emailed to ${to.join(', ')}`, JSON.stringify({ to, cc, subject, status: nextStatus })).run();
        await c.env.DB.prepare(`INSERT INTO client_notifications (id,client_id,project_id,type,title,message) VALUES (?,?,?,?,?,?)`)
            .bind('cn-' + Date.now(), invoice.client_id, invoice.project_id, 'invoice', `Invoice Sent: ${invoice.invoice_number}`, `Invoice ${invoice.invoice_number} was emailed to ${to.join(', ')}${cc.length ? ` with CC to ${cc.join(', ')}` : ''}.`).run();
...
invoices.post('/', async (c) => {
    try {
        const user = c.get('user');
        if (user.role !== 'admin')
            return c.json({ error: 'Only Super Admin can create invoices' }, 403);
        ...
        if (!project_id || !client_id || !title || !amount || !due_date || !issue_date)
...
invoices.put('/:id', async (c) => {
    try {
        const user = c.get('user');
        if (user.role !== 'admin')
            return c.json({ error: 'Only Super Admin can update invoices' }, 403);
        const id = c.req.param('id');
