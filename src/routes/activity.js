...
activity.get('/', async (c) => {
    try {
        ...
        if (client_id) {
            sql += ' AND al.project_id IN (SELECT id FROM projects WHERE client_id=?)';
...
activity.post('/project/:project_id/update', async (c) => {
    try {
        ...
        if (is_client_visible) {
            const proj = await c.env.DB.prepare('SELECT client_id FROM projects WHERE id=?').bind(project_id).first();
            if (proj?.client_id) {
