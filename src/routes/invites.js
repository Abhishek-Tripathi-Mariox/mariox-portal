...
invites.post('/', authMiddleware, requireRole('admin', 'pm'), async (c) => {
    try {
        ...
        if (!email || !full_name)
            return c.json({ error: 'email and full_name required' }, 400);
        ...
        await c.env.DB.prepare("UPDATE user_invites SET expires_at = datetime('now') WHERE email = ? AND accepted_at IS NULL").bind(email.toLowerCase().trim()).run();
...
invites.delete('/:id', authMiddleware, requireRole('admin', 'pm'), async (c) => {
    try {
        await c.env.DB.prepare("UPDATE user_invites SET expires_at = datetime('now') WHERE id = ?").bind(c.req.param('id')).run();
...
invites.get('/validate/:token', async (c) => {
    ...
    `).bind(token).first();
        if (!row)
            return c.json({ valid: false, error: 'Invalid invite token' }, 404);
        if (row.accepted_at)
            return c.json({ valid: false, error: 'This invite has already been used' }, 410);
        if (new Date(row.expires_at) < new Date())
            return c.json({ valid: false, error: 'This invite has expired' }, 410);
        return c.json({
            valid: true,
