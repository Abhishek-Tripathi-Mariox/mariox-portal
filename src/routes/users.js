...
function getRoleFilter(role) {
    ...
    if (normalized === 'pm')
        return { clause: 'u.role IN (?, ?)', values: ['pm', 'pc'] };
    if (normalized === 'developer')
        return { clause: 'u.role IN (?, ?)', values: ['developer', 'team'] };
    if (normalized === 'admin')
        return { clause: 'u.role = ?', values: ['admin'] };
    if (normalized === 'client')
        return { clause: 'u.role = ?', values: ['client'] };
    if (normalized === 'pc' || normalized === 'team')
        return { clause: 'u.role = ?', values: [normalized] };
    return null;
}
...
users.post('/', requireRole('admin'), async (c) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, email, passwordHash, fullName, role, body.phone || null, body.designation || null, body.tech_stack ? JSON.stringify(body.tech_stack) : null, body.skill_tags ? JSON.stringify(body.skill_tags) : null, body.joining_date || null, body.daily_work_hours || 8, body.working_days_per_week || 5, body.hourly_cost || 0, body.monthly_available_hours || 160, body.reporting_pm_id || null, body.avatar_color || '#6366f1', body.remarks || null).run();
...
users.put('/:id', requireRole('admin', 'pm'), async (c) => {
    try {
      ...
      WHERE id=?
    `).bind(body.full_name, body.phone, body.designation, body.tech_stack ? JSON.stringify(body.tech_stack) : null, body.skill_tags ? JSON.stringify(body.skill_tags) : null, body.daily_work_hours || 8, body.working_days_per_week || 5, body.hourly_cost || 0, body.monthly_available_hours || 160, body.reporting_pm_id || null, body.remarks || null, body.is_active !== undefined ? (body.is_active ? 1 : 0) : 1, id).run();
...
users.get('/:id/utilization', async (c) => {
    ...
    `).bind(id, startDate, endDate).first();
        // Count holidays
