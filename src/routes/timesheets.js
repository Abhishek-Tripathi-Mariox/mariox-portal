...
timesheets.post('/', async (c) => {
    try {
        ...
        const assignment = await c.env.DB.prepare('SELECT * FROM project_assignments WHERE user_id = ? AND project_id = ? AND is_active = 1').bind(targetUserId, body.project_id).first();
        ...
        const existing = await c.env.DB.prepare('SELECT COUNT(*) as count FROM timesheets WHERE user_id = ? AND project_id = ? AND date = ?').bind(targetUserId, body.project_id, body.date).first();
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, targetUserId, body.project_id, body.date, body.module_name || null, body.task_description, body.hours_consumed, body.is_billable !== false ? 1 : 0, body.extra_hours_reason || null, body.status || 'in_progress', body.blocker_remarks || null, user.role === 'developer' ? 'pending' : 'approved').run();
...
timesheets.put('/:id', async (c) => {
    try {
        ...
        const body = await c.req.json();
        const entry = await c.env.DB.prepare('SELECT * FROM timesheets WHERE id = ?').bind(id).first();
        if (!entry)
    ...
    `).bind(body.module_name || null, body.task_description, body.hours_consumed, body.is_billable ? 1 : 0, body.extra_hours_reason || null, body.status || 'in_progress', body.blocker_remarks || null, id).run();
...
timesheets.delete('/:id', async (c) => {
    try {
        ...
        if (user.role === 'developer' && entry.user_id !== user.sub)
            return c.json({ error: 'Forbidden' }, 403);
        if (entry.approval_status === 'approved' && user.role === 'developer')
            return c.json({ error: 'Cannot delete approved entries' }, 403);
...
async function updateConsumedHours(db, projectId, userId) {
