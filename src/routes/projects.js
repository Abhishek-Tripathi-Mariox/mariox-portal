...
projects.post('/', requireRole('admin', 'pm'), async (c) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, body.name, body.code, body.client_name || null, body.description || null, body.project_type || 'development', body.start_date, body.expected_end_date, body.priority || 'medium', body.status || 'active', body.total_allocated_hours || 0, body.estimated_budget_hours || 0, body.team_lead_id || null, body.pm_id || null, body.billable !== undefined ? (body.billable ? 1 : 0) : 1, body.revenue || 0, body.remarks || null).run();
...
projects.put('/:id', requireRole('admin', 'pm'), async (c) => {
    try {
      ...
      WHERE id=?
    `).bind(body.name, body.client_name || null, body.description || null, body.project_type || 'development', body.start_date, body.expected_end_date, body.priority || 'medium', body.status || 'active', body.total_allocated_hours || 0, body.estimated_budget_hours || 0, body.team_lead_id || null, body.pm_id || null, body.billable ? 1 : 0, body.revenue || 0, body.remarks || null, id).run();
...
projects.post('/:id/assign-bulk', requireRole('admin', 'pm'), async (c) => {
    try {
        ...
        if (!Array.isArray(developers))
            return c.json({ error: 'developers must be array' }, 400);
