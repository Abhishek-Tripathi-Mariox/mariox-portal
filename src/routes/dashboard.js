...
dashboard.get('/pm', async (c) => {
    try {
        const [projectStats, devStats, hoursStats, recentLogs, weeklyData, monthlyData, topProjects, utilizationData] = await Promise.all([
            // Project statistics
            c.env.DB.prepare(`
        SELECT 
          COUNT(*) as total,
      ...
      `).all()
        ...
        const utilizationResults = utilizationData.results;
        const overloaded = utilizationResults.filter((d) => {
            const pct = d.monthly_available_hours > 0 ? (d.monthly_consumed / d.monthly_available_hours) * 100 : 0;
            return pct > 100;
        }).length;
        const underutilized = utilizationResults.filter((d) => {
            const pct = d.monthly_available_hours > 0 ? (d.monthly_consumed / d.monthly_available_hours) * 100 : 0;
            return pct < 50;
        }).length;
        return c.json({
            data: {
                ...
                utilization: utilizationResults.map((d) => ({
                    ...d,
