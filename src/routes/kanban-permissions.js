...
kanbanPerms.get('/project/:projectId', async (c) => {
    try {
        ...
        const rows = await c.env.DB.prepare('SELECT * FROM kanban_permissions WHERE project_id = ? ORDER BY role').bind(projectId).all();
...
kanbanPerms.get('/project/:projectId/mine', async (c) => {
    try {
        ...
        if (role === 'developer') {
            const a = await c.env.DB.prepare('SELECT 1 FROM project_assignments WHERE project_id = ? AND user_id = ? AND is_active = 1').bind(projectId, user.sub).first();
            isAssignedDev = !!a;
        }
        const perms = await c.env.DB.prepare('SELECT * FROM kanban_permissions WHERE project_id = ? AND role = ?').bind(projectId, role).first();
...
kanbanPerms.put('/project/:projectId', requireRole('admin', 'pm'), async (c) => {
    try {
        ...
        if (!Array.isArray(permissions))
            return c.json({ error: 'permissions array required' }, 400);
      ...
      `).bind(generateId('kp'), projectId, p.role, p.can_view ? 1 : 0, p.can_create_task ? 1 : 0, p.can_edit_any_task ? 1 : 0, p.can_edit_own_task ? 1 : 0, p.can_move_task ? 1 : 0, p.can_delete_task ? 1 : 0, p.can_manage_columns ? 1 : 0, p.can_comment ? 1 : 0).run();
        }
...
export async function checkKanbanPerm(db, projectId, userRole, userId, requiredPerm, taskAssigneeId) {
    ...
    if (userRole === 'developer' && requiredPerm !== 'can_comment') {
        const a = await db.prepare('SELECT 1 FROM project_assignments WHERE project_id = ? AND user_id = ? AND is_active = 1').bind(projectId, userId).first();
        if (!a)
            return { allowed: false, reason: 'not_assigned_to_project' };
    }
    const row = await db.prepare('SELECT * FROM kanban_permissions WHERE project_id = ? AND role = ?').bind(projectId, userRole).first();
