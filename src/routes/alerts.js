...
alerts.get('/', async (c) => {
    ...
    `;
        ...
        const unreadCount = await c.env.DB.prepare('SELECT COUNT(*) as count FROM alerts WHERE is_read=0 AND is_dismissed=0').first();
...
alerts.post('/generate', requireRole('admin', 'pm'), async (c) => {
    ...
    `).all();
        for (const p of burnProjects.results) {
